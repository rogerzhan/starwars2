...
void generateOneTravelRecord(int crewID, int request_dep_aptID, int request_arr_aptID, char *request_earliest_dep, char *request_latest_arr, int request_flight_purpose, int updatetype, int *maxrequestid);
...
writeOutputData(MY_CONNECTION *conn, int scenarioid)
{
	...
	char off_acID_str[32], on_acID_str[32];
...
*/
	for (i=0; i<numPropMgdLegs; i++) {
		...
		sprintf(sqlBuf, "insert into proposed_managedleg values(NULL,%s,%s,%s,%d,%d,%d,%d,'%s','%s')",
...
*/
  ...
  */
	...
	fprintf(logFile,"+--------------+--------------+------------------+--------------+------------------+-----------------+------------+-------------+----------------+\n");
	fprintf(logFile,"| CrewID       | Depart_AptID | Early_Dpt_time   | Arrive_AptID | Late_Arr_time    |  Flight_purpose |  Off_ACID  |   On_ACID   |   Scenario_ID  |\n");
	fprintf(logFile,"+--------------+--------------+------------------+--------------+------------------+-----------------+------------+-------------+----------------+\n");
	if(travelRqstRoot){
	  for(tmp=Minimum(travelRqstRoot);tmp;tmp=Successor(tmp)){
		 ...
		 fprintf(logFile,"| %-12d | %-12d | %10s | %-12d | %10s | %-15d | %10s | %11s |\n",
				trlrqtPtr->crewID,
				...
				trlrqtPtr->on_aircraftID? on_acID_str: "null" //,scenarioid  --comment out by RLZ 04102009
				);
		 
		 sprintf(sqlBuf, "insert into proposed_travelrequest values(null,%d,%d,'%s',%d,'%s',%d, %s, %s,%d)",
			    trlrqtPtr->crewID,
				trlrqtPtr->dept_aptID_travel,
				dt_DateTimeToDateTimeString(dt1, opbuf1, "%Y/%m/%d %H:%M"),
				trlrqtPtr->arr_aptID_travel,
	...
	if(optParam.updateforSimu == 1) writeDataforSimu(conn, scenarioid);
}
...
writeDataforSimu(MY_CONNECTION *conn, int scenarioid)
{
	...
	sprintf(sqlBuf1,"FROM optimizer_results where local_scenarioid = %d) and crewid in (select distinct crewid from proposed_crewassignment where scenarioid = %d)", scenarioid, scenarioid);
	strcat(sqlBuf, sqlBuf1);
    if(!(myDoWrite(conn,sqlBuf))) {
		logMsg(logFile,"%s Line %d: db errno: %d: %s\n", __FILE__,__LINE__,conn->my_errno, conn->my_error_msg);
		writeWarningData(myconn); exit(1);
...
typedef enum { travel_requestid = 0, travel_crewID, travel_dep_aptid, travel_earliest_dep, travel_arr_aptid,
   travel_latest_arr, travel_flight_purpose, travel_from_acID, travel_to_acID, travel_scenario
...
{ char deleteCtrlUnnece[1028], maxrequestidSQL[256], travel_insertSQL[1024], travel_updateSQL[1024], existing_travel_flightsSQL[2048];
  ...
  MYSQL_ROW row;
  int crewID;
  int request_dep_aptID, request_arr_aptID, maxrequestID, request_flight_purpose;
  ...
  sprintf(deleteCtrlUnnece, "delete csflt.* from cs_travel_flights csflt,\n\
                                ...
                                where tmp.request_id = csflt.request_id and tmp.traveller_id = csflt.traveller_id and\n\
								      tmp.departure_time > date_format(date_add('%s', interval %d+12 hour), '%%Y/%%m/%%d %%H:%%i')",
		  paramVal, optParam.travelcutoff);
                                          ...
                                          (select cstrlflight.traveller_id, cstrlflight.request_status, min(departure_gmt_time) as departure_time,  max(arrival_gmt_time) as arrival_time, request_id\n\
								           from\n\
									          ...
									          (SELECT distinct a.crewID\n\
                                               FROM proposed_travelrequest a,  cs_travel_flights b where a.scenarioid = %d and a.crewID = b.traveller_id)\n\
  ...
  sprintf(travel_insertSQL, "select * from proposed_travelrequest where scenarioid = %d and crewid not in\n\
                        (SELECT distinct a.crewID\n\
                        FROM proposed_travelrequest a,  cs_travel_flights b where a.scenarioid = %d and a.crewID = b.traveller_id)",scenarioid, scenarioid);
  ...
  for(rows = 0; rows < rowCount; rows++) {
	...
	generateOneTravelRecord(crewID, request_dep_aptID, request_arr_aptID, request_earliest_dep, request_latest_arr, request_flight_purpose, 0, &maxrequestID);
  }
  mysql_free_result(res);
  ...
  if(! rowCount) {
   ...
   for(rows = 0; rows < rowCount; rows++){
    ...
    request_arr_aptID = atoi(row[travel_arr_aptid]);
    request_latest_arr = strdup(row[travel_latest_arr]);
	generateOneTravelRecord(crewID, request_dep_aptID, request_arr_aptID, request_earliest_dep, request_latest_arr, request_flight_purpose, 1, &maxrequestID);
   }
...
where\n\
   ...
   sprintf(strSQL4," UTC_departure_time >='%s' and\n\
	UTC_arrival_time <= '%s'\n\
    order by UTC_arrival_time asc\n\
	limit 6",
    request_earliest_dep,
    request_latest_arr);
  ...
  if(!rowCount){
    if ((DateTime_dptTm = dt_StringToDateTime("YMDHm", "%d/%d/%d %d:%d", request_earliest_dep, NULL, &errNbr)) == BadDateTime) {
...
where\n\
	arr_aptid in %s and\n\
	UTC_departure_time >='%s' and\n\
	UTC_arrival_time <='%s' order by UTC_arrival_time asc",
	    tmp_str1,
...
********************************************************************************/
 void updateOneTravelRecord(GeneratedCsTravel *gencstrlPtr, int flight_purpose)
 {
	 ...
	 CsTravelData *cstrlBuf, *cstrlPtr, *cstrlPtr1, *cstrlPtr2, *tmp_cstrlPtr;
	 BINTREENODE *tmp;
	 DateTime dt_pwStart;
		 ...
		 for(; tmp; tmp = Successor(tmp)) {
			...
			if(Successor(tmp)){
               ...
               if(tmp_cstrlPtr->crewID == gencstrlPtr->travellerid)
				    ...
				    endwithsameID = 1;
			if (cstrlPtr->travel_dptTm <= dt_pwStart){
				if(endwithsameID == 1){
