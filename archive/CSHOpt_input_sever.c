...
static int readACTypeList(void)
{
	...
	for(x = 0; x < wc; x++) {
		...
		acTypeList[x].macOprCost = atof(paramVal);
	}
...
getDutySoFarAlt(PRE_Crew *preCrewPtr)
{
	...
	//RLZ: we identify some expection cases before coming into this function, related to Travel data.
	if(preCrewPtr->availDuringPlanningWindow != 0){
		if(radPtr){
...
{  char *ODSQL="select OD_index, Org_ID, Dest_ID, Early_Dpt, Late_Arr, Cost from ODTable";
   char *OAGSQL="select OD_index, Departure_time, Arrival_time, Conn_AptID, unAvailable from OAGTable where Conn_AptID < 10000 and OD_index not in (961,3724,8940)";
   int index;
   int errNbr;
   ...
   if(!myDoQuery(myconn, ODSQL, &res, &cols)) 
   {
	...
	{ row = mysql_fetch_row(res);
		...
		if((DateTime_earlyDpt = dt_StringToDateTime("YMDHm", "%d/%d/%d %d:%d", row[field_earlyDpt], NULL, &errNbr)) == BadDateTime) 
	      { logMsg(logFile,"%s Line %d, Bad date in readOagODTable(). errNbr=%d, OD index = %d, date string=%s\n",__FILE__,__LINE__, errNbr, index, row[field_earlyDpt]);
		    writeWarningData(myconn); exit(1);
	      }
        ...
        if((DateTime_lateArr = dt_StringToDateTime("YMDHm", "%d/%d/%d %d:%d", row[field_lateArr], NULL, &errNbr)) == BadDateTime) 
	      { logMsg(logFile,"%s Line %d, Bad date in readOagODTable(). errNbr=%d, OD index = %d, date string=%s\n",__FILE__,__LINE__, errNbr, index, row[field_lateArr]);
		    writeWarningData(myconn); exit(1);
	      }
	...
	{ row_oag = mysql_fetch_row(res_oag);
	   ...
	   popoagptr->connAptID=(row_oag[field_connAptID]? atoi(row_oag[field_connAptID]) : 0);
	    ...
	    if((DateTime_DptTm = dt_StringToDateTime("YMDHm", "%d/%d/%d %d:%d", row_oag[field_dptTm], NULL, &errNbr)) == BadDateTime) 
	     { logMsg(logFile,"%s Line %d, Bad date in readOagODTable()-OAG. errNbr=%d, OD_index = %d, row_count = %d,  date string=%s\n",__FILE__,__LINE__, errNbr, atoi(row_oag[field_odIndex]), rows_oag, row_oag[field_dptTm]);
		    writeWarningData(myconn); exit(1);
	     }
