...
static int getAddlDemandData1(void)
{
	...
	for(i=0; i<numDemand; i++)
	{
		...
		if (demandList[i].recoveryFlag){
			demandList[i].aircraftTypeID = 6;
		    demandList[i].sequencePosn = 3;			
...
static int createAppointDemand(void)
{
	...
	{	
		...
		//RLZ 10/29/2007: BUT can not kick it out of planning window
			...
			if(difftime(maintList[x].endTm,maintList[x].startTm) > (optParam.turnTime + 5)*60){
				demandList[i].reqOut = maintList[x].startTm + (optParam.turnTime - 1)*60; //RLZ 10/29/2007 1 minutes torance for not overlapping in promote.
				demandList[i].reqIn = maintList[x].endTm;
			}
			else{
				demandList[i].reqIn = maintList[x].endTm + (optParam.turnTime + 5)*60 - (int)difftime(maintList[x].endTm,maintList[x].startTm);
				demandList[i].reqOut = demandList[i].reqIn - (5 + 1)*60; 
			}
		}
		else{
...
static int getAddlDemandData2(void)
{
	...
	{	
		...
		else if(demandList[i].aircraftID == 0)
		{
			...
			while(acTypeList[j].sequencePosn < demandList[i].sequencePosn && j<numAcTypes)//acTypeList is sorted by seqencePosn
				j++;
			startTypeIndex = j;
			...
			while(maxTypeIndex > -1 && acTypeList[maxTypeIndex].sequencePosn > (demandList[i].sequencePosn + acTypeList[startTypeIndex].maxUpgrades))
				maxTypeIndex--;
