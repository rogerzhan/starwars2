...
static int readACTypeList(void)
{
	...
	acTypeList[0].capacity = 6;
	acTypeList[1].capacity = 8;
	acTypeList[2].capacity = 6;
	acTypeList[3].capacity = 9;
	acTypeList[4].capacity = 9;

	if(verbose) {
		fprintf(logFile,"+---------+------------+---------+------------+-----------+-----------+------------+------------+\n");
		fprintf(logFile,"| AC TYPE | AC TYPE ID | SEQ POS | CHRTR COST | OPER COST | TAXI COST |    STD REV | MAXUPGRADE | \n");
		fprintf(logFile,"+---------+------------+---------+------------+-----------+-----------+------------+------------+\n");
		for(x = 0; x < wc; x++) {
			fprintf(logFile,"| %7s | %10d | %7d | %10.2f | %9.2f | %9.2f | %10.2f | %10d |\n",
				wptrs[x], acTypeList[x].aircraftTypeID, acTypeList[x].sequencePosn,
				acTypeList[x].charterCost, acTypeList[x].operatingCost,
				acTypeList[x].taxiCost, acTypeList[x].standardRevenue, acTypeList[x].maxUpgrades); 
		}
		fprintf(logFile,"+---------+------------+---------+------------+-----------+-----------+------------+------------+\n");
	}
	numAcTypes = wc;
...
********************************************************************************/
typedef enum {
	mtcAircraftmtcnoteid = 0, mtcAircraftid, mtcAirportid, mtcStarttime, mtcEndtime, mtcApptType, mtc_end_of_list = 255
} mtcSqlColumns;
static int readMaintList(MY_CONNECTION *myconn)
{
...
********************************************************************************/
typedef enum {
	dmdDemandid = 0, dmdContractid, dmdOutaptid, dmdOutfboid, dmdInaptid, dmdInfboid, dmdOuttime,
	dmdIntime, dmdFlexschedule, dmdAircrafttypeid, dmdSequenceposition, dmdAc_type, dmdNbrpsgrs,
	dmdOutcountryid, dmdIncountryid, dmd_end_of_list = 255
} dmdSqlColumns;
static int readDemandList(MY_CONNECTION *myconn)
{
...
+--------+--------+-----+--------+--------+--------+--------+--------+--------+------------------+------------------+----------+----------+----------+-------+-------+-------+-------|\n");
		fprintf(logFile,"\
| demand |  owner | num | outAir | outApt | outFbo | inAir  | inApt  |  inFbo |                  |                  | contract | aircraft | sequence |   Out |    In | Early |  Late |\n");
		fprintf(logFile,"\
|     ID |     ID | Pax | portID | ICAO   |     ID | portID | ICAO   |     ID |           reqOut |            reqIn | Flag     | TypeID   |     Posn | CtyId | CtyId |   Adj |   Adj |\n");
		fprintf(logFile,"\
+--------+--------+-----+--------+--------+--------+--------+--------+--------+------------------+------------------+----------+----------+----------+-------+-------+-------+-------|\n");
		for(x = 0, tPtr = demandList; x < rowCount; ++x, ++tPtr) {
			fprintf(logFile,"| %6d | %6d | %3d | %6d | %-6s | %6d | %6d | %-6s | %6d | %s | %s | %8d | %8d | %8d | %5d | %5d | %5d | %5d |\n",
				tPtr->demandID,
				tPtr->contractID,
				tPtr->numPax,
				tPtr->outAirportID,
...
readCrewList(MY_CONNECTION *myconn)
{
	...
	BINTREENODE *tmp, *tmp1, *tmp2; //, *tmp3, *tmp4;
				  ...
				  {	if (tmpDate >= dt_addToDateTime(Hours, -1, ssCdPtr->dtdate) && tmpDate <= dt_addToDateTime(Hours, 24, ssCdPtr->dtdate)) 
				    { // -1: one hour is a tolerance in the current schedule. so that we do not need to create pseudo ssoft record. RLZ 11/19/2007
						...
						{   // find crew assignment on the TRAVEL day, make it available. RLZ
								...
								sprintf(writetodbstring1, "Warning: Crew %s is assigned on TRAVEL day %s", cdPtr->fileas, dt_DateTimeToDateTimeString(tmpDate, tbuf1, "%Y/%m/%d %H:%M")); 
                           if(errorNumber==0)
						    {if(!( errorinfoList= (Warning_error_Entry *) calloc((size_t) 1, sizeof(Warning_error_Entry)))) 
		                       {logMsg(logFile,"%s Line %d, Out of Memory in readCrewList().\n", __FILE__,__LINE__);
		                        writeWarningData(myconn); exit(1);
...
getTourStartEnd(PRE_Crew *preCrewPtr)
{
	...
	for(tmp = Minimum(preCrewPtr->ssCrewDataRoot); tmp; tmp = Successor(tmp)) {
		...
		if(gmtDayInSSdayRange(pwStartDate, ssCdPtr->dtdate, pwEndDate) && strlen(ssCdPtr->zacccodeid) == 0 && !cantFly(lbuf))
			break;
	}
	if(tmp) {
		...
		for(tmp = Minimum(preCrewPtr->ssCrewDataRoot); tmp; tmp = Successor(tmp)) {
			ssCdPtr = (SS_CrewData *) getTreeObject(tmp);
			if(gmtDayInSSdayRange(pwStartDate, ssCdPtr->dtdate, pwEndDate)) {
				strcpy(lbuf, returnUpper(ssCdPtr->zpostdesc));
				if(strstr(lbuf,"REC NOT IN"))
					strcpy(lbuf, "flying");
...
//	mysql_free_result(res);
	...
	for(tmp = Minimum(dmdKeyWdRoot); tmp; tmp = Successor(tmp)) {
		...
		//TEMPZHAN
		//if (dmdPtr->recoveryFlag){
		//	diPtr->smallestAllowableSeqPos = 3;
		//}
		...
		////TEMPZHAN
		//if (dmdPtr->recoveryFlag){
		//	dmdPtr->aircraftTypeID = 6;
		//	dmdPtr->sequencePosn = 3;
		//}
...
getPeakDayAdjustment(Demand *dPtr)
{
	...
	dPtr->earlyAdj = dPtr->lateAdj = 180;
}
...
{ 
  ...
  if (preCrewPtr->on2)
  {
	  ...
	  { // Make on2 close to pwS within 1 day range 
		  ...
		  fprintf(logFile,"Warning: Making crew assignment (on2) for crewID %5d.", preCrewPtr->crewID);
		  making_on2_flag=1;
	  }
  }
...
*	Purpose:	Add fake demands to bring pilot home
*	Output :	Added fake demands to demand list
****************************************************************************************************/
