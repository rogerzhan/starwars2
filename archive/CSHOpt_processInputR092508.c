		...
		if(!demandList[i].isAppoint){//if NOT a maintenance or appointment leg
			...
			for(j = startTypeIndex; j <= maxTypeIndex; j++)
			{
				...
				if( (demandList[i].aircraftTypeID != acTypeList[j].aircraftTypeID && 
					...
					demandList[i].feasible = 1;
					//MAC - 08/19/08 ANG
					demandList[i].macOprCost[j] = ((demandList[i].blockTm[j] - optParam.taxiInTm - optParam.taxiOutTm)*acTypeList[j].macOprCost)/60 + acTypeList[j].taxiCost;
					if(demandList[i].isMacDemand == 1)
						...
						demandList[i].macOwnerCost[j] = demandList[i].macOprCost[j];
					//
				}
				else //get flight and elapsed times from flight calculator, including taxi times
				{
					...
					//MAC - 08/19/08 ANG
					demandList[i].macOprCost[j] = (flightTm*acTypeList[j].macOprCost)/60 + (numStops+1)*acTypeList[j].taxiCost;
					if(demandList[i].isMacDemand == 1)
					...
					if(demandList[i].incRevStatus[j] != -1){
						...
						demandList[i].macOprCost[j] -= demandList[i].incRev[j]; //MAC - 08/26/08 ANG
