...
static int getAddlDemandData2(void)
{
	...
	{	
		...
		else if(demandList[i].aircraftID == 0)
		{
			...
			if(demandList[i].outAirportID == demandList[i].inAirportID && demandList[i].contingecnyfkdmdflag!=1)
		...
		if(!demandList[i].isAppoint){//if NOT a maintenance or appointment leg
			...
			for(j = startTypeIndex; j <= maxTypeIndex; j++)
			{
				...
				if( (demandList[i].aircraftTypeID != acTypeList[j].aircraftTypeID && 
					...
					demandList[i].early[j] = (int)(demandList[i].reqOut / 60);  
					demandList[i].late[j] = (int)(demandList[i].reqOut / 60); 
					//if(!demandList[i].contingecnyfkdmdflag)
					...
					//else
					  // demandList[i].cost[j] = (optParam.taxiInTm + optParam.taxiOutTm)*acTypeList[j].operatingCost/60 + acTypeList[j].taxiCost;
					demandList[i].feasible = 1;
					//if(demandList[i].incRevStatus[j] == 20){
                    //    getctgncyincev(j, demandList[i].contingencyidx, demandList[i].demandnuminzone, &incRev);
					//	test = incRev;
					//	demandList[i].cost[j] -= incRev*incRev_tune; //Jintao: Benefits to contingency		
					//}
				}
...
populateAcSchedLegIndList(void)
{
	...
	for(k = 0; k<numAircraft; ++k){
		//if(mcpAircraftList[k].numCrPairs >= 2){
