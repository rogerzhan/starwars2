...
*/
  ...
  */
	//comment out for supply analysis - 20090603 - ANG
	/*
	logMsg(logFile,"\n Travel requests:\n");
	fprintf(logFile, "flight_purpose: 1 for on tour, 2 for off tour, 3 for changing plane.\n");
	fprintf(logFile,"+--------------+--------------+------------------+--------------+------------------+-----------------+------------+-------------+-------------+-------------+------------------+----------------+\n");
	fprintf(logFile,"| CrewID       | Depart_AptID | Early_Dpt_time   | Arrive_AptID | Late_Arr_time    |  Flight_purpose |  Off_ACID  |   On_ACID   |   buyticket | cancel_tix  + rqtid_cancelled  +   Scenario_ID  |\n");
	fprintf(logFile,"+--------------+--------------+------------------+--------------+------------------+-----------------+------------+-------------+-------------+-------------+------------------+----------------+\n");
	if(travelRqstRoot){
	  for(tmp=Minimum(travelRqstRoot);tmp;tmp=Successor(tmp)){
		 ...
		 fprintf(logFile,"| %-12d | %-12d | %10s | %-12d | %10s | %-15d | %10s | %11s | %-11d | %-11d | %-16s |\n",
				trlrqtPtr->crewID,
				...
				trlrqtPtr->tixquestid_cancelled? requestid_cancelled_str : "null" //, scenarioid
				);
		 
		 sprintf(sqlBuf, "insert into proposed_travelrequest values(null,%d,%d,'%s',%d,'%s',%d, %s, %s,%d,%d, %s,%d)",
			    trlrqtPtr->crewID,
				trlrqtPtr->dept_aptID_travel,
				dt_DateTimeToDateTimeString(dt1, opbuf1, "%Y/%m/%d %H:%M"),
				trlrqtPtr->arr_aptID_travel,
...
writeDataforSimu(MY_CONNECTION *conn, int scenarioid)
{
	...
	//sprintf(sqlBuf1, " FROM optimizer_results where local_scenarioid = %d)", scenarioid);
    //strcpy(sqlBuf,"delete from crewassignment where endtime >= (SELECT date_format(planning_window_start, '%Y/%m/%d %H:%i')"); - MODIFIED FOR SUPPLY ANALYSIS - 2009/06/10 ANG
