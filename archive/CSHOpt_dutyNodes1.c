		...
		{	//for each aircraftType
			...
			{	//check if infeasible due to sequence position or fleet exclusion
				...
				{	
					...
					tmpInv = optParam.dutyNodeAdjInterval;
					...
					if( optParam.dutyNodeAdjInterval > 180 && optParam.planningWindowDuration < 3){
						tmpInv = 60;
						flexStart = -(int)((intReqOut - max(demandList[i].early[j], (int)optParam.windowStart/60))/tmpInv);
					    flexEnd = (int)((demandList[i].late[j] - intReqOut)/tmpInv);
					}
					...
					for (flexInc = flexStart; flexInc <= flexEnd; flexInc++){ 
						...
						newDuty->startTm[0] = (int)(demandList[i].reqOut)/60 + tmpInv*flexInc; 
...
//RLZ 04/11/2008, need to consider the combination of the duty
	...
	if (early[0]> 0 && (optParam.dutyNodeAdjInterval <= 180 || optParam.planningWindowDuration < 3 )){
		early[0] = 	late[0] = (int) (demandList[exgDuty->demandInd[0]].reqOut/60);
	}
