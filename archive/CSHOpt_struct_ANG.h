...
typedef struct networkArc
{
	...
	double onMacCost; //if the destination node is put on a Mac - MAC - 08/19/08 ANG
	double onRegCost; //if the destination node is put on a regular aircraft - MAC - 08/19/08 ANG
...
typedef struct duty
{
	...
	double onMacCost; //Cost of putting a Mac demand on Mac - MAC - 08/19/08 ANG
	double onRegCost; //Cost of putting a Mac demand on regular aircraft - MAC - 08/19/08 ANG
	double actualOnMacCost; //onMacCost without early/late penalty - MAC - 08/19/08 ANG
	double actualOnRegCost; //onRegCost without early/late penalty - MAC - 08/19/08 ANG
} Duty;
...
//	int maxUpgrades; //maximum number of sequencePositions that a demand leg can be upgraded
	...
	int exgCPACLock;   //flag for not re assign existing crew pairs
} OptParameters;
...
//typedef struct mcpAircraft //mcp: multi crew-pairs - 03/06/08 ANG
//{
//	int numCrPairs;
//	int cprInd[5];
//} McpAircraft;
