char *svnKeyWords[] = {
"$LastChangedDate: 2008-09-11 17:13:08 -0400 (Thu, 11 Sep 2008) $",
"$Revision$",
"$LastChangedBy: jouyang $",
"$HeadURL: http://csintranetdev01/repos/trunk/optimizer/cmdWin/CSHOpt_mainDir/CSHOpt_main.c $",
"$Id: CSHOpt_main.c 12051 2008-09-11 21:13:08Z jouyang $",
...
CmdLineVar commandLineVars[] = {
	...
	{ "macPenalty", (char *) 0 },
	{ "macOprCostMultiple", (char *) 0 },
	{ "macOwnerCostMultiple", (char *) 0 },
...
main(int argc, char **argv)
{
	...
	while(--argc) 
	{
        ...
        else if(strncasecmp("--macOprCostMultiple=", arg, 21) == 0)
    		setCmdLineVar("macOprCostMultiple", arg + 21); 
        else if(strncasecmp("--macOwnerCostMultiple=", arg, 23) == 0)
    		setCmdLineVar("macOwnerCostMultiple", arg + 23); 
		else {
...
getParameters(char *params)
{
   ...
   if(oracleDirect == 0){
	...
	for(x = 0; x < vc; ++x) {
		dt = dt_addToDateTime(Hours, optParam.travelcutoff + 12, dt_run_time_GMT); //need to get travelcutoff into parameters file
   ...
   /////////////////////////////////
	...
	if(oracleDirect != 0){
		...
		for(x = 0; x < vc; ++x) {
			...
			if(strncasecmp("travelcutoff", *(variables + x), 12) == 0)
			  optParam.travelcutoff = atoi(paramVal);   
			if(!s1) {
				logMsg(logFile,"substitution failed: substitute() returned null, substitution count = %d\n", sc);
				exit(1);
