...
//	else
	...
	//readMacInfo(myconn);//MAC - 08/29/08 ANG	
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	/* required: "macOprCostMultiple" - MAC - 09/05/08 ANG*/
	if(!(paramVal = getCmdLineVar("macOprCostMultiple"))) {
	  if(!(paramVal = getParamValue("macOprCostMultiple"))) {
		logMsg(logFile,"Required parameter \"macOprCostMultiple\" missing from parameter file.\n");
		writeWarningData(myconn); exit(1);
	  }
	}
	optParam.macOprCostMultiple = atof(paramVal);
	if(verbose) fprintf(logFile,"%27s = %8.2f\n", "macOprCostMultiple", optParam.macOprCostMultiple);

	/* required: "macOwnerCostMultiple" - MAC - 09/05/08 ANG*/
	if(!(paramVal = getCmdLineVar("macOwnerCostMultiple"))) {
	  if(!(paramVal = getParamValue("macOwnerCostMultiple"))) {
		logMsg(logFile,"Required parameter \"macOwnerCostMultiple\" missing from parameter file.\n");
		writeWarningData(myconn); exit(1);
	  }
	}
	optParam.macOwnerCostMultiple = atof(paramVal);
	if(verbose) fprintf(logFile,"%27s = %8.2f\n", "macOwnerCostMultiple", optParam.macOwnerCostMultiple);
...
static int readACTypeList(void)
{
	...
	// _ac_type_id, _seq_pos, _chrtr_cost, _oper_cost, _taxi_cost _std_rev
	for(x = 0; x < wc; x++) {
		strcpy(vbuf, wptrs[x]);
	...
	if(verbose) {
		fprintf(logFile,"+---------+------------+---------+------------+-----------+-----------+------------+------------+----------+---------+-----------+\n");
		fprintf(logFile,"| AC TYPE | AC TYPE ID | SEQ POS | CHRTR COST | OPER COST | TAXI COST |    STD REV | MAXUPGRADE | CAPACITY | UPRCVRY | DOWNRCVRY |\n");
		fprintf(logFile,"+---------+------------+---------+------------+-----------+-----------+------------+------------+----------+---------+-----------+\n");
		for(x = 0; x < wc; x++) {
			fprintf(logFile,"| %7s | %10d | %7d | %10.2f | %9.2f | %9.2f | %10.2f | %10d | %8d | %7d | %10d |\n",
				wptrs[x], acTypeList[x].aircraftTypeID, acTypeList[x].sequencePosn,
				acTypeList[x].charterCost, acTypeList[x].operatingCost,
	...
	//MAC - TEMP - 08/19/08 ANG
	//Temporary values for macOprCost and macOwnerCost
	for(x = 0; x < wc; x++) {
		acTypeList[x].macOprCost = optParam.macOprCostMultiple * acTypeList[x].operatingCost;
		acTypeList[x].macOwnerCost = optParam.macOwnerCostMultiple * acTypeList[x].operatingCost;
	}
...
readACList(MY_CONNECTION *myconn)
{
	...
	char tbuf[32];
//	char tbuf1[32];// 02/28/08 ANG
  char writetodbstring1[200];
	int a; //MAC - 09/02/08 ANG  
	...
	for(tPtr = tmpACList, tPtr2 = acList;tPtr->aircraftID;tPtr++) {
		...
		//if (tPtr->aircraftID == 1450 ||
		//	tPtr->aircraftID == 1567 ||
		//	tPtr->aircraftID == 1586 )
		//	tPtr->isMac = 1;
		//else
		//	tPtr->isMac = 0;
		//		
...
static int readMaintList(MY_CONNECTION *myconn)
{
	...
	if(verbose) {
		...
		fprintf(logFile,"+---------------+------------+-----------+------------------+------------------+----------+\n");
		for(x = 0, tPtr = maintList; x < rowCount; ++x, ++tPtr) {
	...
	numMaintenanceRecord = (int) rowCount;
...
static int readDemandList(MY_CONNECTION *myconn)
{
	...
	for(rows = 0, tPtr = demandList; rows < rowCount; rows++) { // 09/09/08 ANG
		...
		//MAC - TEMP - 08/19/08 ANG
		//Identify Mac demands by checking owner and contract IDs in macInfoList
		tPtr->isMacDemand = 0;
		for(a = 0; a < numMac; a++){
			if(tPtr->ownerID == macInfoList[a].ownerID && tPtr->contractID == macInfoList[a].contractID){
				tPtr->isMacDemand = 1;
...
//###################################################################################################################
       ...
       //populate the last travel leg for crew
		...
		if(preCrewPtr->availDuringPlanningWindow) {
			...
			if(optParam.withCTC)
			  getDutySoFarWithCTC(preCrewPtr);
			else
...
//			}                                                                              //debug
		...
		//TEST
		tmpDate = dt_addToDateTime(Hours, -preCrewPtr->startEarly, preCrewPtr->tourStartTm);
	...
	if(verbose) {
		...
		fprintf(logFile,"+-------+------------------+------------------+------------------+------------------+-------+------------------+----+-------+-------+------+------------------+------------------+\n");
		fprintf(logFile,"|crewid |    On1           |   Off1           | On2              | Off2             |avaiAPT| cs_avaiDT        | CL | DutyT |BlockT | Base | tourStart        | tourEnd          |\n");
		fprintf(logFile,"+-------+------------------+------------------+------------------+------------------+-------+------------------+----+-------+-------+------+------------------+------------------+\n");
		preCrewPtr = preCrewList;
		while(preCrewPtr->employeenumber) {
			if(preCrewPtr->availDuringPlanningWindow == 0) {
...
static void showTravelLeg(CsTravelData *cstrlPtr){								
	...
	fprintf(logFile,"| %8d | %6d | %6d | %s | %s | \n",
		...
		(cstrlPtr->travel_arrTm) ? dt_DateTimeToDateTimeString(cstrlPtr->travel_arrTm, dbuf1, "%Y/%m/%d %H:%M") : "0000/00/00 00:00");
}
...
showCombined_CrewDataPreCrew_OnOff(PRE_Crew *preCrewPtr)
{
	...
	fprintf(logFile,"| %4d | %s | %s | %s | %s | %4d  | %s | %2d | %5d | %5d | %4d | %s | %s |  \n",
	preCrewPtr->crewID,
...
/****************************************************************************************************
*	Function	getDutySoFarAlt							Date last modified:  10/29/07 ANG
...
*	Notes  :	Only applies to crew with the following characteristics:
*				- both lastActivityLeg and lastActivityLeg2 are populated
*				- lastActivityLeg is a flight leg
...
getDutySoFarAlt(PRE_Crew *preCrewPtr)
{
	...
	//RLZ: still needs to try to find altAvailDT
	//if(preCrewPtr->availDuringPlanningWindow != 0){
		if(radPtr){
...
validateCrewAircraftAssociationFlightLeg(char *empnbr, DateTime outtime, DateTime intime, int aircraftid, char *rowType)
{
	...
	case NotFoundReturnedNextItem:
		for(; tmp; tmp = Successor(tmp)) {
			...
			if(strcmp(crwPtr->employeenumber, empnbr) == 0) {
				if(crwPtr->aircraftid == aircraftid) {
					if(outtime >= crwPtr->starttime && intime <= crwPtr->endtime) //Intersect 
						return(1);
				}
			}
			else
...
fprintf(logFile,"+---------+-----+------+------------------+------------------+-------+-------+-------+-------+------------------+------+-------+-------+-----+\n");
fprintf(logFile,"|         |     | ac   | tour             | tour             | start |  stay | end   | avail |                  | act  | duty  | block | cat |\n");
fprintf(logFile,"|  crewID | pos | type | starttime        | endtime          | Early |  late | loc   | apt   | availDT          | code | time  | time  |  id |\n");
fprintf(logFile,"+---------+-----+------+------------------+------------------+-------+-------+-------+-------+------------------+------+-------+-------+-----+\n");
}
...
showCrew(Crew *cPtr)
{
	...
	fprintf(logFile,"| %7d | %3d | %4d | %s | %s | %5.2f | %5.2f | %5s | %5s | %s | %4d | %s | %s | %3d |\n",
		cPtr->crewID,
		cPtr->position,
		cPtr->aircraftTypeID,
		dt_DateTimeToDateTimeString(dt_StringToDateTime("sDHmY", "%*s %s %d %d:%d:%*d %d\n",
...
static int
getDutySoFarWithCTC(PRE_Crew *preCrewPtr)
{
	RawAircraftData *radLastLegPtr;
	CsTravelData *cstrlPtr;
	DateTime dt_pwStart, dt_runTime, flight_in_time, on2Info, off2Info;
	...
	if(radLastLegPtr && cstrlPtr){
		...
		if(strcmp(radLastLegPtr->rowtype,"mtcnote") == 0 || strcmp(radLastLegPtr->rowtype,"ownersign") == 0){
			if(cstrl_dptTm > dt_pwStart &&
				(radLastLegPtr->rec_outtime <= dt_pwStart) && 
				    (radLastLegPtr->rec_intime >= dt_pwStart) &&
				    (radLastLegPtr->rec_intime <= dt_addToDateTime(Minutes, optParam.maintTmForReassign, dt_pwStart))){
				cstrlPtr = NULL;
				cstrl_ignored = 1;
...
//	if(radLastLegPtr&&(! cstrlPtr||(radLastLegPtr->rec_intime >= cstrl_arrTm)&&(strcmp(radLastLegPtr->rowtype,"logmgdleg") == 0 || strcmp(radLastLegPtr->rowtype,"mgdleg") == 0))){
	if(radLastLegPtr&&(! cstrlPtr||(radLastLegPtr->rec_intime >= cstrl_arrTm ||((radLastLegPtr->rec_intime < dt_pwStart) && (radLastLegPtr->rec_outtime >= dt_addToDateTime(Hours, -24*(int)preCrewPtr->startEarly, preCrewPtr->tourStartTm))&& cstrl_dptTm > dt_pwStart))&&(strcmp(radLastLegPtr->rowtype,"logmgdleg") == 0 || strcmp(radLastLegPtr->rowtype,"mgdleg") == 0))){
		if(strcmp(radLastLegPtr->rowtype,"logmgdleg") == 0 || strcmp(radLastLegPtr->rowtype,"mgdleg") == 0){
			//the last leg is a flight leg
			if(!preCrewPtr->on1 && verbose){
				fprintf(logFile, "\n Found crew with lastActivityLeg = 'logmgdleg' or 'mgdleg' but no (ON1,OFF1) information: \n");
 ...
 ********************************************************************************/
typedef enum {
	acid, ctid, owid, mac_end_of_list = 255
} macSqlColumns;
static int
readMacInfo(MY_CONNECTION *myconn)
{
	...
	for(rows = 0; rows < rowCount; rows++) {
		...
		if(! row[acid]) 
			continue;
		macInfoList[numMac].aircraftID = atoi(row[acid]);
		macInfoList[numMac].contractID = atoi(row[ctid]);
		macInfoList[numMac].ownerID = atoi(row[owid]);
		numMac++;
	}
	...
	if(verbose) {
		fprintf (logFile, "\n There are %d Mac: \n", numMac);
		fprintf(logFile,"+------------+------------+----------+\n");
		fprintf(logFile,"| aircraftID | contractID | ownerID  |\n");
		fprintf(logFile,"+------------+------------+----------+\n");
		for(a = 0; a < numMac; a++) {
			fprintf(logFile,"| %10d | %10d | %8d |\n", macInfoList[a].aircraftID, macInfoList[a].contractID, macInfoList[a].ownerID);
		}
		fprintf(logFile,"+------------+------------+----------+\n");
		fflush(logFile);
	}
...
readMacInfoFromOracleDB(ORACLE_SOCKET *orl_socket)
{
	...
	while(Orlconnection_fetch(orl_socket)==0) {
		macInfoList[numMac].aircraftID = atoi(orl_socket->results[acid2]);
		macInfoList[numMac].contractID = atoi(orl_socket->results[ctid2]);
		macInfoList[numMac].ownerID = atoi(orl_socket->results[owid2]);
		numMac++;
	}
