...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	/* required: "groundTravelTm" */
	if(!(paramVal = getCmdLineVar("groundTravelTm"))) {
     	if(!(paramVal = getParamValue("groundTravelTm"))) {
		  logMsg(logFile,"Required parameter \"groundTravelTm\" missing from parameter file.\n");
		  writeWarningData(myconn); exit(1);
	    }
	}
	optParam.groundTravelTm = atoi(paramVal);
	if(verbose) fprintf(logFile,"%27s = %5d\n", "groundTravelTm", optParam.groundTravelTm);
	...
	for(x = 0, mPtr = maintList; x < numMaintenanceRecord; ++x, ++mPtr) {
		if(mPtr->aircraftID && mPtr->airportID){
			if( addMe == 1  && mPtr->aircraftID == currentAircraftID
							&& mPtr->airportID == getBaseToCsiMapping(preCrewPtr->startLoc)
							&& (mPtr->startTm <= DateTimeToTime_t(dt_addToDateTime(Minutes, 2*optParam.turnTime, endTime))
							||  mPtr->endTm >= DateTimeToTime_t(dt_addToDateTime(Minutes, -2*optParam.turnTime, endTime)))){
				...
				if( mPtr->aircraftID == currentAircraftID // 11/12/07 ANG
					...
					fmPtr->airportID = getBaseToCsiMapping(preCrewPtr->startLoc);
...
//	CrewData *cdptr;
	...
	{ cstrl_arrTm = dt_addToDateTime(Minutes, optParam.postArrivalTime, cstrlPtr->travel_arrTm);
	  //cstrl_arrTm = dt_addToDateTime(Minutes, optParam.groundTravelTm, cstrl_arrTm);
