...
addCharterCols (const CPXENVptr env, const CPXLPptr lp)
{
	...
	for (i=0; i<numDemandRows; i++) // Add one column for each demand row.
	{
		...
		else // Not an appointment/maintenance demand
		{
			...
			if(demandList[demandRowIndex[i]].contingecnyfkdmdflag ==1)
			{  for(actypeidx = 0; actypeidx < MAX_AC_TYPES; actypeidx++)
			   {  if(demandList[demandRowIndex[i]].aircraftTypeID == acTypeList[actypeidx].aircraftTypeID)
			      break;
			   }
			  if(actypeidx < MAX_AC_TYPES)
			  { getctgncyincev(actypeidx, demandList[demandRowIndex[i]].contingencyidx, demandList[demandRowIndex[i]].demandnuminzone, &incRev);
		        cost = cost*tune_parameters + incRev;
			  }
			  else
			  { fprintf(logFile, "can't find aircraftid %d in addCharterCols.\n", demandList[demandRowIndex[i]].aircraftTypeID);
			  }
			}
			#ifdef DEBUGGING
