...
static int readACTypeList(void)
{
	...
	if((paramVal = getCmdLineVar("maxUpgrades"))) {
		...
		if(wcUpgrades != 5) { //CX - 12/04/2009 ANG
			...
			//writeWarningData(myconn); exit(1); //temp fix - 12/21/2009 ANG
		}
		for(x = 0; x < wcUpgrades; ++x) {
			acTypeList[x].maxUpgrades = atoi(wptrsUpgrades[x]);
		}
		//acTypeList[wc-1].maxUpgrades = 0; //Highest fleet, no upgrades //temp fix - 12/21/2009 ANG
		if (wcUpgrades == 4 && x == 4) acTypeList[4].maxUpgrades = 0; //temp fix - 12/21/2009 ANG
		//acTypeList[wc-1].maxUpgrades = 0;
	...
	acTypeList[wc - 2].upgradeRecovery = 1; //SOV
...
static int readDemandList(MY_CONNECTION *myconn)
{
	...
	//readRecoveryDemandList(&recoveryDemandNum); RLZ for no flex
...
// end debug ////////////////////////////////////////////////////////////////////////////
	...
	while(preCrewPtr->employeenumber) {
		...
		//01/28/2010 RLZ temp FIX)
		//   if (preCrewPtr->crewID == 24327 ||preCrewPtr->crewID == 66301)
		//    	preCrewPtr->aircraftTypeID = 52;
		//END TEMP FIX
		}
		++preCrewPtr;
	}
...
analyseAircraftType(PRE_Crew *pre_crewPtr)
{
	...
	int returnValues[] = { 5, 6, 54, 11, 52, 20, 0 };
...
//			  for each plane above that: incremental = R * ratio_of_guaranteed_approved_plane * (time_in_flown - time_in_guaranteed)
	...
	fprintf(logFile,"| g |   CJ1 |  Brav |   CJ3 |   Exc |    Sov |     CX | Aircraft     | OwnerId | Id      | Demand Out Time  | Out Time         |      Id |      Id | Name     | Pos | TYP | Name       | Pos | TYP | Name       | Keywords\n"); //CX - 12/04/2009 ANG
	fprintf(logFile,"+---+-------+-------+-------+-------+--------+--------+--------------+---------+---------+------------------+------------------+---------+---------+----------+-----+-----+------------+-----+-----+------------+\n");
 ...
 ********************************************************************************/
typedef enum {
	acid, actpid, ctid, macdoc, mac_end_of_list = 255
} macSqlColumns;
static int
...
void writeToIncrementalRevenueTable(MY_CONNECTION *myconn, int scenarioid)
{
	...
	for(tmp = Minimum(dmdKeyWdRoot); tmp; tmp = Successor(tmp)) {
		...
		//for(i = 0; i < (MAX_AC_TYPES - 1); i++) {
  		  if(i < MAX_AC_TYPES -1){ //Remove when moved to production - CX - 12/04/2009 ANG
			if(dmdPtr->incRevStatus[i] == -1){
				sprintf(tempBuf, "0, ");
 ...
 ********************************************************************************/
typedef enum {
	crid, actypeid1, actypeid2, dq_end_of_list = 255
} dqSqlColumns;
static int
readDualQualCrew(MY_CONNECTION *myconn)
{
	...
	if(verbose) {
		...
		fprintf(logFile,"+--------+-----------+-----------+\n");
		fprintf(logFile,"| crewID | acTypeID1 | acTypeID2 |\n");
		fprintf(logFile,"+--------+-----------+-----------+\n");
		for(a = 0; a < numDQCrew; a++) {
