...
identifyImpossiblePairings (void)
{
	...
	if (verbose)
	{
		...
		fprintf (pairCrewsLogFile,"+--------+--------+------------+\n");
		fprintf (pairCrewsLogFile,"| Crew1  | Crew 2 |   Value    |\n");
		fprintf (pairCrewsLogFile,"+--------+--------+------------+\n");
		...
		for (i=0; i<numCrew; ++i)
		{
			for (j=0; j<numCrew; ++j)
			{
				fprintf (pairCrewsLogFile, "| %6d | %6d | %10.3f |\n", 
					crewList[i].crewID,
					crewList[j].crewID,
					crewPairMatrix[i][j]);
			}
...
computePairingBenefit (const int p1, const int p2)
{
	...
	if (verbose)
	{
		fprintf (pairCrewsLogFile, "| %6d | %6d | %10.3f | (Change penalty)\n",
			crewList[p1].crewID,
			crewList[p2].crewID,
			benefit);
	}
	...
	if (verbose)
	{
		fprintf (pairCrewsLogFile, "| %6d | %6d | %10.3f | (Beyond planning window benefit)\n",
			crewList[p1].crewID,
			crewList[p2].crewID,
			benefit);
	}
	...
	if (verbose)
	{
		fprintf (pairCrewsLogFile, "| %6d | %6d | %10.3f | (Close pilot base benefit)\n",
			crewList[p1].crewID,
			crewList[p2].crewID,
			benefit);
	}
...
buildNewCrewPairList (void)
{
	...
	if ((newCrewPairList = (CrewPair *) calloc (ncp, sizeof (CrewPair))) == NULL)
	{
...
buildNewCrewPairListWithoutPairing (void)
{
	...
	if ((newCrewPairList = (CrewPair *) calloc (numCrewPairs, sizeof (CrewPair))) == NULL)
	{
...
computeSwapBenefit (void)
{
	...
	for (i=0; i<numAircraft; i++)
	{
		...
		if(acList[i].firstCrPrID == 0)
			continue;  //no need to calculate swap benefit
...
enumerateCrewSwaps (void)
{
	...
	*/
	...
	if (verbose)
	{
		...
		fprintf (pairCrewsLogFile, "+--------+--------+--------+--------+------------------+------------------+------------+\n");
		fprintf (pairCrewsLogFile, "| APT ID | AC ID  | CapID  | FO ID  |    Swap Start    |     Swap End     |   Benefit  |\n");
		fprintf (pairCrewsLogFile, "+--------+--------+--------+--------+------------------+------------------+------------+\n");
		for (i=0; i<numCrewSwaps; ++i)
		{
...
computeTotalBenefit (int i, int j, int k, int x, int y)
{
	...
	if (latestArrivalTm + 60*remainingPairDutyTime < (crewSwaps[k].lastSchedInDuty + optParam.postFlightTm*60))
...
buildOptProblem (void)
{
	...
	if ((oag = (OAG **) calloc ((2 * 24 * 60 * MAX_WINDOW_DURATION)/TIME_STEP+1, sizeof (OAG *))) == NULL)
	{
		logMsg(logFile,"%s Line %d: Out of Memory in buildOptProblem().\n", __FILE__,__LINE__);
		writeWarningData(myconn); exit(1);
	}
	for (i=0; i<(2* 24 * 60 * MAX_WINDOW_DURATION)/TIME_STEP+1; i++)
	{
	...
	if (verbose)
	{
		...
		while (temp) {
			...
			fprintf(pairCrewsLogFile, "| %6d | %6d |  %6d  | %6d  | %15s | %10.3f |\n", 
				crewList[temp->p1].crewID, crewList[temp->p2].crewID, temp->ac, temp->apt, 
				dt_DateTimeToDateTimeString(dt, opbuf, "%Y/%m/%d %H:%M"), temp->benefit);
			temp = temp->next;
		}
		fprintf (pairCrewsLogFile, "+--------+--------+----------+---------+------------------+------------+\n"); 
	}
...
compareCrewLists ()
{
	...
	fprintf (compareFile, "+--------------+----------------+----------------+\n");
	fprintf (compareFile, "| Crew Pair ID |   Captain ID   |  Flight Off ID |\n"); 
	fprintf (compareFile, "+--------------+----------------+----------------+\n");
	for (i=0; i<numOldCrewPairs; i++)
	{
		fprintf (compareFile, "|    %6d    |     %6d     |     %6d     |\n", oldCrewPairList[i].crewPairID, oldCrewPairList[i].captainID,
			oldCrewPairList[i].flightOffID);
	}
	fprintf (compareFile,       "+--------------+----------------+----------------+\n");
	...
	{
		fprintf (compareFile, "|    %6d    |     %6d     |     %6d     |\n", crewPairList[i].crewPairID, crewPairList[i].captainID,
			crewPairList[i].flightOffID);
	}
	fprintf (compareFile,       "+--------------+----------------+----------------+\n");
...
showOptimalMatching (void)
{
	...
	fprintf (pairCrewsLogFile, "\n\nOptimal matching:\n\n");
	fprintf (pairCrewsLogFile, "+--------+--------+--------+--------+------------------+------------+\n");
	fprintf (pairCrewsLogFile, "|Aircraft|Airport | Pilot1 | Pilot2 |     Swap time    |   Benefit  |\n");
	fprintf (pairCrewsLogFile, "+--------+--------+--------+--------+------------------+------------+\n");
	for (i=0; i<numMatchingArcs; i++)
	{
		if (optMatching[i])
