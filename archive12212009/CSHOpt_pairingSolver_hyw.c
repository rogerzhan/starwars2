...
optimizeIt (MatchingArc *matchingArcs, int numMatchingArcs, int *optMatching)
{
	...
	while (temp)
	{
		...
		index[2] = numUniquePilots + getIndex (temp->ac, uniqueAC, numUniqueAC); //WNG
	    ...
	    status = CPXaddcols (env, lp, 1, 2, &benefit, &zeroInt, index, coeff,  &zeroDbl, &oneDbl, NULL); //WNG
