...
identifyImpossiblePairings (void)
{
	...
	if (verbose)
	{
		...
		for (i=0; i<numCrew; ++i)
		{
			for (j=0; j<numCrew; ++j)
			{
				fprintf (pairCrewsLogFile, "| %6d | %6d | %6d | %6d | %10.3f |\n", // DQ - 12/09/2009 ANG
					crewList[i].crewID,
					crewList[j].crewID,
	...
	if (verbose)
	{
		...
		for (i=0; i<numCrew; ++i)
		{
			for (j=0; j<numCrew; ++j)
			{
				fprintf (pairCrewsLogFile, "| %6d | %6d | %6d | %10.3f |\n", 
					crewList[i].crewID,
					crewList[j].crewID,
					crewList[i].aircraftTypeID,
					crewList[j].aircraftTypeID,
...
compareCrewLists ()
{
	...
	fprintf (compareFile, "+--------------+----------------+----------------+----------------+\n");
	fprintf (compareFile, "| Crew Pair ID |   Captain ID   |  Flight Off ID |    acTypeID    |\n"); 
	fprintf (compareFile, "+--------------+----------------+----------------+----------------+\n");
	for (i=0; i<numOldCrewPairs; i++)
	{
		fprintf (compareFile, "|    %6d    |     %6d     |     %6d     |     %6d     |\n", oldCrewPairList[i].crewPairID, oldCrewPairList[i].captainID,
...
reprintFinalCrewPairList (void)
{
	...
	for(cp = 0; cp < numOptCrewPairs; cp++){ 
			fprintf(logFile,"| %4d | %4d | %6d | %6d | %6d | %s | %s | %3d |", 
				cp, 
