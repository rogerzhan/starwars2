CREATE PROCEDURE `proc_updateCrewdutydate`()
    MODIFIES SQL DATA
BEGIN
	...
	IF (select count(*) from CrewSchedule_KINGAIR)>0 THEN  -- make sure the connection exists before delete the data
		...
		(
			...
			(
				...
				from
				(
					select id, trim(col_6) as crewCode, case when col_5 like '%dual%' or col_5 like '%fusion train%' then 3 when col_5 like '%fusion%' then 2 else 1 end as qualification from CrewSchedule_KINGAIR where length(trim(col_6)) =4
				) c
				inner join
				(
					select id, date_format(str_to_date((select col_7 from CrewSchedule_KINGAIR where id = 3),'%m/%d/%Y'), '%Y/%m/%d %H:%i') as dutyDate, col_7 gsValue from CrewSchedule_KINGAIR union all
					select id, date_format(str_to_date((select col_8 from CrewSchedule_KINGAIR where id = 3),'%m/%d/%Y'), '%Y/%m/%d %H:%i') as dutyDate, col_8 gsValue from CrewSchedule_KINGAIR union all
					select id, date_format(str_to_date((select col_9 from CrewSchedule_KINGAIR where id = 3),'%m/%d/%Y'), '%Y/%m/%d %H:%i') as dutyDate, col_9 gsValue from CrewSchedule_KINGAIR union all
					select id, date_format(str_to_date((select col_10 from CrewSchedule_KINGAIR where id = 3),'%m/%d/%Y'), '%Y/%m/%d %H:%i') as dutyDate, col_10 gsValue from CrewSchedule_KINGAIR union all
					select id, date_format(str_to_date((select col_11 from CrewSchedule_KINGAIR where id = 3),'%m/%d/%Y'), '%Y/%m/%d %H:%i') as dutyDate, col_11 gsValue from CrewSchedule_KINGAIR union all
					...
					select id, date_format(str_to_date((select col_351 from CrewSchedule_KINGAIR where id = 3),'%m/%d/%Y'), '%Y/%m/%d %H:%i') as dutyDate, col_351 gsValue from CrewSchedule_KINGAIR
				) d
				on c.id = d.id
			...
			(
				select a.regionPosition, a.rowId as beginRowId, ifnull(b.rowId, 1000) as nextRowId, 
						case 
							...
							when left(a.regionPosition, locate(' ',a.regionPosition) - 1) ='CHECKAIRMEN' then 'CA'
				...
				(
					...
					from
					(
						select id as rowId , case when length(col_5)>0 then col_5 else col_2 end as regionPosition
						from CrewSchedule_KINGAIR 
						where (length(col_6) is null or length(col_6) < 4) -- no crewCode
						and (upper(col_2) in ('NORTHEAST PIC','NORTHEAST SIC','SOUTHEAST PIC','SOUTHEAST SIC','MIDWEST PIC ', 'MIDWEST PIC','MIDWEST SIC','WEST PIC','WEST SIC','CHECKAIRMEN','STANDARD CAPT.','IOE') or upper (col_5) in('FUSION ONLY-N857UP and Beyond','FUSION ONLY SIC'))
						order by id
					) t, ( select @rownum := 0 ) r
				) a
				left join
			...
			(
				...
				from
				(
					select id, trim(col_6) as crewCode, null as qualification from CrewSchedule_EXCEL where length(trim(col_6)) =4
				) c
				inner join
				(
					select id, date_format(str_to_date((select col_7 from CrewSchedule_EXCEL where id = 3),'%m/%d/%Y'), '%Y/%m/%d %H:%i') as dutyDate, col_7 gsValue from CrewSchedule_EXCEL union all
					select id, date_format(str_to_date((select col_8 from CrewSchedule_EXCEL where id = 3),'%m/%d/%Y'), '%Y/%m/%d %H:%i') as dutyDate, col_8 gsValue from CrewSchedule_EXCEL union all
					select id, date_format(str_to_date((select col_9 from CrewSchedule_EXCEL where id = 3),'%m/%d/%Y'), '%Y/%m/%d %H:%i') as dutyDate, col_9 gsValue from CrewSchedule_EXCEL union all
					select id, date_format(str_to_date((select col_10 from CrewSchedule_EXCEL where id = 3),'%m/%d/%Y'), '%Y/%m/%d %H:%i') as dutyDate, col_10 gsValue from CrewSchedule_EXCEL union all
					select id, date_format(str_to_date((select col_11 from CrewSchedule_EXCEL where id = 3),'%m/%d/%Y'), '%Y/%m/%d %H:%i') as dutyDate, col_11 gsValue from CrewSchedule_EXCEL union all
					...
					select id, date_format(str_to_date((select col_351 from CrewSchedule_EXCEL where id = 3),'%m/%d/%Y'), '%Y/%m/%d %H:%i') as dutyDate, col_351 gsValue from CrewSchedule_EXCEL
				) d
				on c.id = d.id
				where d.gsValue between 1 and 9 OR d.gsValue in ('OT', 'A/OT', 'AOT', 'AOT/VAC')
			) c
