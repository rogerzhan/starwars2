...
#include "DutyLine.h" //fei DL
#include "params.h"//ft test
#include "split.h" //ft test
...
extern time_t run_time_t; //ft test
...
int getDemForUpgrade( MY_CONNECTION *conn ); //up test
int isDemPotentialSale(const int ind); //up test

int solveBeforeFit(CPXENVptr env, CPXLPptr lp ); //ft test
static int modifyCost (double *costP); //ft test
static int getAddInfoForAfterFitSoln( CPXENVptr env, CPXLPptr lp );//ft test

int showAllExgTours( const ExgTour *exgTourList, const int numExgTours ); //test
...
ExgTour *optExgTours = NULL;//fei DL
static int numOptTours = 0;
int numOptExgTours = 0;//fei DL
...
//			cost = acTypeList[i].charterCost*(max(optParam.minCharterTime, demandList[demandInd].elapsedTm[i]))/60.0;
	...
	if(optParam.withMac == 1){
		if(!optParam.charterMacDmd && demandList[demandInd].isMacDemand)
			return 3 * cost;
	}

	//For trips having the same origin and destination airports, do not charter - 11/03/11 ANG
	if(demandList[demandInd].outAirportID == demandList[demandInd].inAirportID){
		return 3 * cost;
	}
		...
		{// If maintenance/appointment, charter cost is uncovMaintPenalty.
			...
			switch (origDemInfos[demandRowIndex[i]].isAppoint){
				case 1:
					...
					break;
				case 2:
					if (origDemInfos[demandRowIndex[i]].origSkipIncl)
...
addExgSolnCols (const CPXENVptr env, const CPXLPptr lp)
{
	...
	showAllExgTours( exgTourList, numExgTours );//test
...
changeVariableType (CPXENVptr env, CPXLPptr lp)
{
	...
	//numCols = CPXgetnumcols (env, lp); //ft test
	//avoid trying to use global variable numCols here; number of columns should be derived from (env, lp)
	int lNumCols = CPXgetnumcols (env, lp);

	if ((ctype = (char *) calloc (lNumCols, sizeof (char))) == NULL)
	{
			logMsg(logFile,"%s Line %d: Out of Memory in changeVariableType().\n", __FILE__,__LINE__);
			writeWarningData(myconn); exit(1);
	}		

	if ((indices = (int *) calloc (lNumCols, sizeof (int))) == NULL)
	{
			logMsg(logFile,"%s Line %d: Out of Memory in changeVariableType().\n", __FILE__,__LINE__);
			writeWarningData(myconn); exit(1);
	}		

	if ((lu = (char *) calloc (lNumCols, sizeof (char))) == NULL)
	{
			logMsg(logFile,"%s Line %d: Out of Memory in changeVariableType().\n", __FILE__,__LINE__);
			writeWarningData(myconn); exit(1);
	}		

	if ((bd = (double *) calloc (lNumCols, sizeof (double))) == NULL)
	{
			logMsg(logFile,"%s Line %d: Out of Memory in changeVariableType().\n", __FILE__,__LINE__);
			writeWarningData(myconn); exit(1);
	}			
	...
	status = CPXchgbds (env, lp, lNumCols, indices, lu, bd);
	if ( status ) {
...
//END TEMP (UNCOMMENT NEXT LINE BACK IN)
	...
	//ft test 
	//numCols, solution and optSolution: associated with the after-fit LP; OUTPUT will be generated based on them
	...
	if (status = CPXgetmipx (env, lp, solution, 0, numCols-1) ) 
	{
	...
	getAddInfoForAfterFitSoln(env, lp ); //ft test
	solveBeforeFit( env, lp ); //ft test
...
getOptTourPlanes (void)
{
	...
	//logMsg(logFile, "numCols = %d, numInitialCols = %d\n",numCols,numInitialCols);

	for (i=0; i<numCols-numInitialCols; i++) // new tours are contained in numInitialCols .. numCols-1
	{
		if (optSolution[numInitialCols+i])
		{
			//fprintf(logFile,"  tourList[%d].crewPairInd = %d ",i, tourList[i].crewPairInd);
			optTours[numOptTours] = tourList[i];
			//fprintf(logFile,"--> optTours[%d].crewPairInd = %d ",numOptTours, optTours[numOptTours].crewPairInd);
			numOptTours ++;
			//fprintf(logFile,"  numOptTours = %d \n", numOptTours);
			//fflush(logFile);
		}
	}
...
static int constructManagedLegsFA (void)
{
	...
	{ // Look through optTourList and optAircraftID and spit out data.
		...
		fprintf(logFile, "\n\n Tour: %d --> \n AC: %d, CP index: %d, Crew: %d, Crew: %d --> Cost: %f \n", i, acID, cpInd
		, crewPairList[cpInd].captainID, crewPairList[cpInd].flightOffID, optTours[i].cost );
		...
		{   
			...
			for (k=0; k<maxTripsPerDuty; k++)
			{
				...
				{// If location where plane is available is not the outAirport, create managed repositioning leg. 
					...
					propMgdLegs[numPropMgdLegs].demInd = -1 ; //up test
				...
				if (demandList[lastDemandInd].isAppoint == 0)// Create managed demand leg if not a maintenance/appointment leg
				{
					...
					propMgdLegs[numPropMgdLegs].demInd = lastDemandInd; //up test
...
//END TEMP
		...
		for (j=0; j<numLegs; j++)
		{
				...
				&& (!legList[j].exgTourException)) //no need to output repo-to-nothing legs in exg tour - 03/31/10 ANG 
			{
				...
				propMgdLegs[numPropMgdLegs].demInd = legList[j].demandInd; //up test
		...
		if (!scanned[i][0])
		{
			...
			if(scannedFirst[c] == 0){ //if this is the first leg we have scanned for the pilot
				...
				if(crewList[c].origActCode == 0 && crewPairList[propMgdLegs[i].crewPairInd].hasFlownFirst == 1 &&
					...
					difftime(propMgdLegs[i].schedOut, crewList[c].origAvailDT) < 60*((crewList[c].origBlockTm? optParam.postFlightTm : 0)+ optParam.minRestTm + acTypeList[crewList[c].acTypeIndex].preFlightTm - 5)) //07/17/2017 ANG
					...
					startTm = propMgdLegs[i].schedOut - 60*acTypeList[crewList[c].acTypeIndex].preFlightTm; //07/17/2017 ANG
			}
			else
				//startTm = propMgdLegs[i].schedOut - 60*optParam.preFlightTm;
				startTm = propMgdLegs[i].schedOut - 60*acTypeList[crewList[c].acTypeIndex].preFlightTm; //07/17/2017 ANG
			...
			for (j=i+1; j<numPropMgdLegs; j++)
			{
				if ((propMgdLegs[j].captainID == crewID) && (propMgdLegs[j].aircraftID == acID) && 
					...
					difftime(propMgdLegs[j].schedOut, endTm) < 60*(optParam.minRestTm + acTypeList[crewList[c].acTypeIndex].preFlightTm - 5)) //07/17/2017 ANG
				{
					...
					scanned[j][0] = 1;
						...
						((propMgdLegs[j].captainID == crewID) && (propMgdLegs[j].aircraftID == acID) && 
							...
							difftime(propMgdLegs[j].schedOut, endTm) >= 60*(optParam.postFlightTm + optParam.minRestTm + acTypeList[crewList[c].acTypeIndex].preFlightTm - 5))) //07/17/2017 ANG
				{
					break;
				}
			}
		...
		if (!trl_scanned[i][0])
		{
			...
			if(break_indicator == 1){
				if(curr_propmdgleg->schedInAptID != next_propmdgleg->schedOutAptID){
					...
					trlrqstPtr->latest_arr = next_propmdgleg->schedOut - 60*acTypeList[crewList[c].acTypeIndex].preFlightTm;//07/17/2017 ANG
					trlrqstPtr->off_aircraftID = curr_propmdgleg->aircraftID;
...
//	int days, hours, minutes, seconds, msecs, timediff;//03/10/08 ANG
	...
	*/
	...
	logMsg(logFile, "numOptTours = %d, numCrewPairs = %d, numAircraft = %d\n", numOptTours, numCrewPairs, numAircraft);
	for(i=0; i < numOptTours; i ++)
	{
		logMsg(logFile, "  i = %i, optTours[i].crewPairInd = %d, optAircraftID[i] = %d\n", i, optTours[i].crewPairInd, optAircraftID[i]);
	}
...
constructOptimalSchedule (void)
{
	...
	logMsg(logFile, "constructOptimalSchedule(), numDemand = %d\n", numDemand);
	logMsg(logFile, "constructOptimalSchedule(), numCrew = %d\n", numCrew);
	logMsg(logFile, "constructOptimalSchedule(), numExgTourCols = %d\n", numExgTourCols);
	...
	if (!optParam.crewAllAc) //Fei DL - 11/08/11 ANG
		if (TRAVEL_REQ_FLAG)  constructTravelRequest ();

	writeProposedSolution ();

	if( optParam.uptest )
		getDemForUpgrade( myconn ); //up test
...
getOptimalSchedule ()
{
	...
	if (optParam.crewAllAc)//Fei DL - 11/08/11 ANG
		dutyLine(&propCrewAssignment, &numPropCrewDays, &travelRqstRoot, &travelRqsts); //fei DL

	//printMaintList();//for debug - 04/22/08 ANG

	freeAndNullMemory (); //AD2017
	return 0;
}
	...
	for (i=0; i<numOptDemand; i++){
		if (!demandList[i].isAppoint){
			if(demandList[i].legInd > 0){
				...
				if (z == numAcTypes){
					logMsg(logFile, "Warning: Revenue by ContractType for Scheduled demandID %d is not populated. \n", demandList[i].demandID);
				}
				//populate by FlownType
				for(z = 0; z < numAcTypes; z++){
				...
				if (z == numAcTypes){
					logMsg(logFile, "Warning: Revenue by FlownType for Scheduled demandID %d is not populated. \n", demandList[i].demandID);
				}
			}
			else{
				...
				if (z == numAcTypes){
					logMsg(logFile, "Warning: Revenue by ContractType and FlownType for Scheduled demandID %d is not populated. \n", demandList[i].demandID);
				}
			}
		}
	}
...
int populateProposedFlexOSandMX (void)
{
	...
	{	
		...
		if ( origDemInfos[i].isAppoint != 2 && origDemInfos[i].isAppoint != 4 && origDemInfos[i].isAppoint != 5)
			continue;
		...
		if( optSolution[i] == 1 ){//uncovered appointments
			...
			//Create propOwnerSigning entries for VAC even if this is uncovered - 11/11/11 ANG
			if(origDemInfos[i].isAppoint == 5){
				propOwnerSigning[numPropOwnerSigning].recordID = origDemInfos[i].uniqueID;
				propOwnerSigning[numPropOwnerSigning].aircraftID = acList[origDemInfos[i].acInd].aircraftID;
				propOwnerSigning[numPropOwnerSigning].airportID = origDemInfos[i].outAptID;
				propOwnerSigning[numPropOwnerSigning].fboID = origDemInfos[i].outFboID;
				propOwnerSigning[numPropOwnerSigning].startTime = origDemInfos[i].defaultOut;
				propOwnerSigning[numPropOwnerSigning].endTime = origDemInfos[i].defaultIn;
	...
	for (i=0; i<numDemand; i++)
	{	
		if ( demandList[i].isAppoint != 2 && demandList[i].isAppoint != 4 && demandList[i].isAppoint != 5)
	...
	//for (i=0; i < numOrigDem; i++){
	for (i=0; i < numOptDemand; i++){
			if(origDemInfos[demandList[i].origDemInd].isAppoint != 2 && origDemInfos[demandList[i].origDemInd].isAppoint != 4 && origDemInfos[demandList[i].origDemInd].isAppoint != 5)
...
//up test
int getDemForUpgrade( MY_CONNECTION *conn )
{
	int i, j, k, ind ;
	double reqRate;
	char sqlBuf[1024], tmbuf[1024], fleetNames[MAX_AC_TYPES][32];
	MYSQL_RES *res;
	MYSQL_FIELD *cols;
	MYSQL_ROW row;
	my_ulonglong rowCount, rows;
	char *demandforupgradeSQL = "create table if not exists uptest_demandtoupgrade \n\
													( \n\
													demandid int(11) not null,  \n\
													requestflighttime decimal(10,2) not null, \n\
													numberpassenger int(2) not null, \n\
													contractfleettype varchar(9) not null, \n\
													requestfleettype varchar(9) not null, \n\
													assignedfleettype varchar(9) not null, \n\
													upgraderestrictioncode int(1) not null, \n\
													preceddemandid int(11), \n\
													succdemandid int(11), \n\
													isinexistingtour int(1), \n\
													contractrequestcharge decimal(10,2) not null, \n\
													ischargeactual int(2) not null, \n\
													constraint primary key (demandid) \n\
													) ";
	char *upgradecontractrateSQL = "create table if not exists uptest_contractupgradecharge \n\
													( \n\
													demandid int(11) not null, \n\
													fleettype varchar(9) not null, \n\
													flighttime decimal(10,2) not null, \n\
													contractupgradecharge decimal(10,2) not null, \n\
													ischargeactual int(2) not null, \n\
													constraint primary key (demandid, fleettype) \n\
													) ";

	if(!(myDoWrite(conn, demandforupgradeSQL))) 
	{
		logMsg(logFile,"%s Line %d: db errno: %d: %s\n", __FILE__,__LINE__,conn->my_errno, conn->my_error_msg);
		exit(1);
	}
	if(!(myDoWrite(conn, "truncate table uptest_demandtoupgrade"))) 
	{
		logMsg(logFile,"%s Line %d: db errno: %d: %s\n", __FILE__,__LINE__,conn->my_errno, conn->my_error_msg);
		exit(1);
	}
	if(!(myDoWrite(conn, upgradecontractrateSQL))) 
	{
		logMsg(logFile,"%s Line %d: db errno: %d: %s\n", __FILE__,__LINE__,conn->my_errno, conn->my_error_msg);
		exit(1);
	}
	if(!(myDoWrite(conn, "truncate table uptest_contractupgradecharge"))) 
	{
		logMsg(logFile,"%s Line %d: db errno: %d: %s\n", __FILE__,__LINE__,conn->my_errno, conn->my_error_msg);
		exit(1);
	}

	if( !myDoQuery(conn,  "select * from fractionalprogram", &res, &cols) || !res )
	{
	...
	_ASSERTE( rowCount > 0 );
	memset(fleetNames, 0, sizeof(fleetNames));
	for(rows = 0; rows < rowCount; rows++) 
	{
		if( !(row = mysql_fetch_row(res)) )
			break;

		for( j=0; j < numAcTypes; j ++ )//find ac type of this row
				if( acTypeList[j].aircraftTypeID == atoi(row[2]) )
					break ;

		if( j >= numAcTypes ) 
			continue;

		_ASSERTE( strlen(row[3])+1 <= sizeof(fleetNames[j]) );
		strcpy(fleetNames[j], row[3] );
	}
	mysql_free_result(res);

	for(j=0; j < numAcTypes; j ++)
		_ASSERTE( j == acTypeList[j].sequencePosn - acTypeList[0].sequencePosn );

	fprintf (logFile, "DemandID, Requested Flight Time, Num Passengers, Contract Type, Requested Type, Actual Type, Restriction Code, Prev Dem, Next Dem, ext tour \n");
	for (i=0; i<numPropMgdLegs; i++)
	{	
		ind = propMgdLegs[i].demInd ;

		if( propMgdLegs[i].demandID <= 0 )
			continue ;

		_ASSERTE( ind >= 0 && demandList[ind].demandID == propMgdLegs[i].demandID );
		
		if(! isDemPotentialSale(ind) )//if either pre dem or suc dem is a potential upgrade sale, then keep current demand
		{
			if( demandList[ind].predDemID )
			{
				for(j=0; j < numDemand; j++)
					if( demandList[j].demandID == demandList[ind].predDemID )
						break;
				_ASSERTE( j < numDemand );
			} else
				j = -1;

			if( demandList[ind].succDemID )
			{
				for(k=0; k < numDemand; k++)
					if( demandList[k].demandID == demandList[ind].succDemID )
						break;
				_ASSERTE( k < numDemand );
			} else
				k = -1;

			if( (j == -1 || !isDemPotentialSale(j)) && (k == -1 || !isDemPotentialSale(j)) )
				continue;
		}
		
		for(j=demandList[ind].sequencePosn - acTypeList[0].sequencePosn; j<numAcTypes; j ++) //get the next feasible fleet type
			if(demandList[ind].flightTm[j] < INFINITY)
				break;
		_ASSERTE( j < numAcTypes && numAcTypes <= MAX_AC_TYPES && demandList[ind].contractRate[j] > 0 );
		reqRate = demandList[ind].flightTm[j]*demandList[ind].contractRate[j]/60.0 ;

		fprintf (logFile, " %d, %s, %.2f, %d, %d, %d, %d, %d, %8d, %8d, %d \n"
		, propMgdLegs[i].demandID
		, dt_DateTimeToDateTimeString(dt_time_tToDateTime (demandList[ind].reqOut), tmbuf, "%Y/%m/%d %H:%M")
		, (demandList[ind].reqIn - demandList[ind].reqOut)/3600.0
		, demandList[ind].numPax
		, demandList[ind].contractSeqPosn
		, demandList[ind].sequencePosn
		, acTypeList[crewPairList[propMgdLegs[i].crewPairInd].acTypeIndex].sequencePosn
		, demandList[ind].upRestrict
		, demandList[ind].predDemID
		, demandList[ind].succDemID
		, propMgdLegs[i].exgTour
		);

		sprintf(sqlBuf, "insert into uptest_demandtoupgrade values(%d,%f,%d,'%s','%s','%s',%d,%d,%d,%d,%f, %d)"
		, propMgdLegs[i].demandID		
		, (demandList[ind].reqIn - demandList[ind].reqOut)/3600.0
		, demandList[ind].numPax
		, fleetNames[demandList[ind].contractSeqPosn-acTypeList[0].sequencePosn]
		, fleetNames[demandList[ind].sequencePosn-acTypeList[0].sequencePosn]
		, fleetNames[crewPairList[propMgdLegs[i].crewPairInd].acTypeIndex]
		, demandList[ind].upRestrict
		, demandList[ind].predDemID
		, demandList[ind].succDemID
		, propMgdLegs[i].exgTour
		, reqRate , demandList[ind].isRateActual[j]//j and reqRate
		);
		...
		for(j=0; j < numAcTypes; j ++)
		{
			_ASSERTE( demandList[ind].flightTm[j] > 0 );
			if( demandList[ind].flightTm[j] == INFINITY )
			{
				_ASSERTE( demandList[ind].flightTm[j] == demandList[ind].blockTm[j] );
				fprintf (logFile, " infeasible, 0, ");
			} else
			{
				_ASSERTE( INFINITY > demandList[ind].blockTm[j] );
				fprintf (logFile, " %.2f, %d, ", demandList[ind].flightTm[j]*demandList[ind].contractRate[j]/60.0, demandList[ind].isRateActual[j] );

				sprintf(sqlBuf, "insert into uptest_contractupgradecharge values(%d, '%s', %f, %f, %d)"
				, propMgdLegs[i].demandID, fleetNames[j], demandList[ind].flightTm[j]/60.0
				, demandList[ind].flightTm[j]*demandList[ind].contractRate[j]/60.0 - reqRate, demandList[ind].isRateActual[j] );
...
//up test
int isDemPotentialSale(const int ind)
{
	if((demandList[ind].numPax <= 4 || demandList[ind].reqIn - demandList[ind].reqOut <= 6000)
	|| demandList[ind].contractSeqPosn != demandList[ind].sequencePosn 
	|| ( demandList[ind].aircraftTypeID != 54 && demandList[ind].aircraftTypeID != 76
	&& demandList[ind].aircraftTypeID != 11 ))
		return 0;
	else
		return 1;
}
...
//ft test
int solveBeforeFit(CPXENVptr env, CPXLPptr lp )
{
	int i, demInd, status ;
	int numStalled=0, perturbed=0;
	//int numCols = CPXgetnumcols (env, lp);
	double *duals, *slnAfFit, *slnB4Fit, newObjValue, curObjValue, objAfFit, costDiff;
	char outFitDemSql[1024];
	int numCharterAfFit, numCharterB4Fit, numColB4Fit;
	const double IntRoundUp=0.5;

	if( !optParam.fitModel )
		return 0;

	_ASSERTE( numRows + numRepoConnxnRows == CPXgetnumrows (env, lp) && numCols == CPXgetnumcols (env, lp));
	srand ((int)time(NULL) );

	//keep obj and current solution
	if (status = CPXgetmipobjval (env, lp, &objAfFit)) 
	{
		logMsg(logFile, "Failed to get objective function value.\n");
		CPXgeterrorstring (env, status, charBuf);
		logMsg(logFile,"%s", charBuf);
	...
	if ((slnAfFit = (double *) calloc (numCols, sizeof (double))) == NULL)
	{
		logMsg(logFile,"%s Line %d: Out of Memory.\n", __FILE__,__LINE__);
		exit(1);
	}	
	if (status = CPXgetmipx (env, lp, slnAfFit, 0, numCols-1) ) 
	{
		logMsg(logFile, "Failed to recover optimal solution.\n");
		CPXgeterrorstring (env, status, charBuf);
		logMsg(logFile,"%s", charBuf);
	...
	//get number of charters
	numCharterAfFit=0;
	for (i=0; i<numDemandRows; i++)
		if( slnAfFit[i] > IntRoundUp)
			numCharterAfFit ++;

	iteration = 0 ;//reset
	if ( status = CPXchgprobtype (env, lp, CPXPROB_LP) ) 
	{
		fprintf (stderr, "Failed to change back to LP \n");
		CPXgeterrorstring (env, status, charBuf);
		logMsg(logFile,"%s", charBuf);
		exit(1);
	}
	//update lp for fit demand
	for (i=0; i<numDemandRows; i++)
	{
		rowInd[i] = i;
		if( optParam.withFlexOS )
			rowCoeff[i] = (origDemInfos[demandRowIndex[i]].fit ? 0 : 1) ;
		else
			rowCoeff[i] = (demandList[demandRowIndex[i]].fit ? 0 : 1) ;
	}
	if (status = CPXchgrhs(env, lp, numDemandRows, rowInd, rowCoeff)) 
	{
		logMsg(logFile, "Failed to change RHS.\n");
		CPXgeterrorstring (env, status, charBuf);
		logMsg(logFile,"%s", charBuf);
	...
	if (!(duals = (double *) calloc (numRows, sizeof (double))))
	{
		logMsg(logFile,"%s Line %d: Out of Memory in optimizeIt().\n", __FILE__,__LINE__);
		exit(1);
	}
	if (status = CPXgetpi (env, lp, duals, 0, numRows-1)) 
	{
		logMsg(logFile, "Failed to get dual prices.\n");
		CPXgeterrorstring (env, status, charBuf);
		logMsg(logFile,"%s", charBuf);
	...
	if (status = CPXgetobjval (env, lp, &curObjValue)) 
	{
		logMsg(logFile, "Failed to get objective function value.\n");
		CPXgeterrorstring (env, status, charBuf);
		logMsg(logFile,"%s", charBuf);
	...
	while (generateNewCols (env, lp, duals)) 
	{
		...
		if (status = CPXgetobjval (env, lp, &newObjValue)) 
		{
			logMsg(logFile, "Failed to get objective function value.\n");
			CPXgeterrorstring (env, status, charBuf);
			logMsg(logFile,"%s", charBuf);
		...
		else if (fabs((newObjValue-curObjValue)/curObjValue) <  EPSILON)
		{
			...
			// Number of consecutive iterations where there has been little improvement in objective
			if ( ++numStalled > MAX_STALLS)
			{
				fprintf(logFile, "Perturbing problem...\n");
				for (i=0; i<numDemandRows; i++)
				{
					rowInd[i] = i;
					if( optParam.withFlexOS )
						rowCoeff[i] = (origDemInfos[demandRowIndex[i]].fit ? 0 : (MIN_RHS + ((double) rand()/ (double) RAND_MAX) * (MAX_RHS - MIN_RHS))) ;
					else
						rowCoeff[i] = (demandList[demandRowIndex[i]].fit ? 0 : (MIN_RHS + ((double) rand()/ (double) RAND_MAX) * (MAX_RHS - MIN_RHS))) ;
					//rowCoeff[i] = MIN_RHS + ((double) rand()/ (double) RAND_MAX) * (MAX_RHS - MIN_RHS);
				}
				if (status = CPXchgrhs(env, lp, numDemandRows, rowInd, rowCoeff)) 
				{
	...
	if (status = CPXgetmipobjval (env, lp, &curObjValue)) 
	{
		logMsg(logFile, "Failed to get objective function value.\n");
		CPXgeterrorstring (env, status, charBuf);
		logMsg(logFile,"%s", charBuf);
	...
	logMsg (logFile, "** Solved MIP: objective function value: %f \n", curObjValue);

	numColB4Fit = CPXgetnumcols (env, lp);
	if ((slnB4Fit = (double *) calloc (numColB4Fit, sizeof (double))) == NULL)
	{
		logMsg(logFile,"%s Line %d: Out of Memory.\n", __FILE__,__LINE__);
		exit(1);
	}	
	if (status = CPXgetmipx (env, lp, slnB4Fit, 0, numColB4Fit-1) ) 
	{
		logMsg(logFile, "Failed to recover optimal solution.\n");
		CPXgeterrorstring (env, status, charBuf);
		logMsg(logFile,"%s", charBuf);
	...
	numCharterB4Fit=0;
	for (i=0; i<numDemandRows; i++)
		if( slnB4Fit[i] > IntRoundUp)
			numCharterB4Fit ++;
	_ASSERTE( numCharterB4Fit <= numCharterAfFit );

	if( optParam.fitModel )
	{
		costDiff = objAfFit - curObjValue ;
		//modifyCost( &costDiff ); //apply a piece wise linear function to the cost difference
		objAfFit = curObjValue + costDiff ;
	}
	...
	for (i=0; i<numDemandRows; i++)
	{
		if( optParam.withFlexOS )
		{
			if( !origDemInfos[demandRowIndex[i]].fit )
				continue ;
			demInd = origDemInfos[demandRowIndex[i]].indices[0];
		} else
		{
			if( !demandList[demInd].fit )
				continue ;
			demInd = i ;
		}
		sprintf(outFitDemSql, "update fit_demand set totalcostbeforefit=%10.2f, totalcostafterfit=%10.2f, numbernewcharter=%d, scenarioid=%d \n\
	    where recordid=%d ", curObjValue, objAfFit, numCharterAfFit-numCharterB4Fit, local_scenarioid, demandList[demInd].fit );

		if(!(myDoWrite(myconn, outFitDemSql))) //myconn global
		{
			logMsg(logFile,"%s Line %d: db errno: %d: %s\n", __FILE__,__LINE__, myconn->my_errno, myconn->my_error_msg);
			exit(1);
		}
	}//end for (i=0; i<numDemandRows; i++)
...
//ft test
static int modifyCost (double *costP)
{
	char sbuf[1024] ;
	int bpCount=0, slopeCount=0 ;//count number of break points and slopes (one slope for a break point)
	char *paramVal, *bpChar[128], *slopeChar[128] ;
	double *bp, *slope, constant=0, newCost=0 ;
	int i, j;

	_ASSERTE( optParam.fitModel );//assume there is only one new fit demand
	//read break points
	if(!(paramVal = getParamValue("fitModelBP"))) 
	{
		logMsg(logFile,"Required parameter \"fitModelBP\" missing from parameter file.\n");
		exit(1);
	}
	strcpy(sbuf, paramVal);
	if( ! (bpCount = split(sbuf," \t", bpChar)) )
	{
		logMsg(logFile,"No break points for the fit demand piece wise linear function \n");
		exit(1);
	}
	if ((bp = (double *) calloc (bpCount, sizeof (double))) == NULL)
	{
		logMsg(logFile,"%s Line %d: Out of Memory.\n", __FILE__,__LINE__);
		exit(1);
	}	
	...
	{
		bp[i] = atof(bpChar[i]) ;
		if( bp[i] <= 0 || (i > 0 && bp[i-1] >= bp[i] ) )
		{
			logMsg(logFile,"Non-increasing break points for the fit demand piece wise linear function \n");
			exit(1);
		}
	}
	
	//read slopes
	if(!(paramVal = getParamValue("fitModelSlope"))) 
	{
		logMsg(logFile,"Required parameter \"fitModelSlope\" missing from parameter file.\n");
		exit(1);
	}
	strcpy(sbuf, paramVal);
	if( !(slopeCount = split(sbuf," \t", slopeChar) ) || slopeCount != bpCount + 1 )//one more slope for the case after the last break point
	{
		logMsg(logFile,"No break points for the fit demand piece wise linear function \n");
		exit(1);
	}
	if ((slope = (double *) calloc (slopeCount, sizeof (double))) == NULL)
	{
		logMsg(logFile,"%s Line %d: Out of Memory.\n", __FILE__,__LINE__);
		exit(1);
	}	
	...
	{
		slope[i] = atof(slopeChar[i]) ;
		if( slope[i] <= 0 )
		{
			logMsg(logFile,"Negative slope for the fit demand piece wise linear function \n");
			exit(1);
		}
	}
	//read constant
	if(!(paramVal = getParamValue("fitModelConst"))) 
	{
			logMsg(logFile,"Required parameter \"fitModelConst\" missing from parameter file.\n");
			exit(1);
	}
	constant = atoi(paramVal) ;

	if( optParam.withFlexOS )
	{
		for(i=0; i < numOrigDem; i ++)
			if( origDemInfos[i].fit )
			{
				for(j=0; j<bpCount; j ++)
					if( origDemInfos[i].defaultOut <= run_time_t + bp[j]*3600 )
					{
						newCost = (*costP)*slope[j] + constant ;
						break;
					}
				newCost = (j < bpCount ? newCost : (*costP)*slope[j] + constant );
				break;
			}
	} else
	{
		for(i=0; i < numDemand; i ++)
			...
			{
				for(j=0; j<bpCount; j ++)
					if( demandList[i].reqOut <= run_time_t + bp[j]*3600 )
					{
						newCost = (*costP)*slope[j] + constant ;
						break;
					}
...
//ft test
static int getAddInfoForAfterFitSoln( CPXENVptr env, CPXLPptr lp )
{
	int i, j, iteration, acTypeInd, k, d, status, numFitDem, fitCount ;
	ExgTour **exgTours=NULL;
	Tour **tours=NULL;
	int nExgTours=0, nTours=0, notFound=1, acInd=-1, pickupType=-1, pickupInd=-1, lNumCols=0, demInd=-1;
	time_t deptTm=0, arrTm=0 ;
	double *x=NULL;
	char tmBuf1[32], tmBuf2[32], outFitDemSql[1024];
	const double IntRoundUp=0.5;
	double docCost=0;
	int *fitDemInd=0;
	...
	_ASSERTE( numFitDem > 0 );
	if ((fitDemInd = (int *) calloc (numFitDem, sizeof (int))) == NULL)
	{
		logMsg(logFile,"%s Line %d: Out of Memory.\n", __FILE__,__LINE__);
		exit(1);
	}	
	...
	if( optParam.withFlexOS )
	{
		for(i=0; i<numOrigDem; i ++)
			if( origDemInfos[i].fit )
			{
				fitDemInd[d] = origDemInfos[i].indices[0] ;//index in demandList
				d ++;
			}
	} else 
	{
	...
	//get solution x (after fit)
	lNumCols = CPXgetnumcols (env, lp);
	if ((x = (double *) calloc (lNumCols, sizeof (double))) == NULL)
	{
		logMsg(logFile,"%s Line %d: Out of Memory.\n", __FILE__,__LINE__);
		exit(1);
	}	
	if (status = CPXgetmipx (env, lp, x, 0, lNumCols-1) ) 
	{
		logMsg(logFile, "Failed to recover optimal solution.\n");
		CPXgeterrorstring (env, status, charBuf);
		logMsg(logFile,"%s", charBuf);
	...
	//count number of optimal existing tours
	nExgTours=0;
	for (i=0; i<numExgTourCols; i++)
		if (x[numCharterCols+i] > IntRoundUp)
			nExgTours ++;
	if (nExgTours && (exgTours = (ExgTour **) calloc (nExgTours, sizeof (ExgTour *))) == NULL)
	{
		logMsg(logFile,"%s Line %d: Out of Memory.\n", __FILE__,__LINE__);
		exit(1);
	}
	...
	for (i=0; i<numExgTourCols; i++)
		if (x[numCharterCols+i] > IntRoundUp)
		{
			exgTours[j] = &(exgTourList[i]) ;
			j ++ ;
		}

	//count number of optimal new tours
	nTours=0;
	for (i=0; i<lNumCols-numInitialCols; i++)
		if (x[numInitialCols+i] > IntRoundUp)
			nTours ++;
	if (nTours  && (tours = (Tour **) calloc (nTours, sizeof (Tour *))) == NULL)
	{
		logMsg(logFile,"%s Line %d: Out of Memory.\n", __FILE__,__LINE__);
		exit(1);
	}
	...
	for(fitCount=0; fitCount < numFitDem; fitCount ++ )
	{
		//find demand (assume this demand won;t be contained in existing tours, so only look it up in the new tours)
		//(aslo assume that this demand is a regular demand)
		deptTm = arrTm = 0;
		acInd = pickupType = pickupInd = -1;
		for (i=0; i<nTours; i++)
		{
			acTypeInd = crewPairList[tours[i]->crewPairInd].acTypeIndex;
			for (j=0; j<MAX_WINDOW_DURATION; j++)
			{
				if ((k = tours[i]->duties[j]) < 0)
					continue;
				for(d = 0; d < maxTripsPerDuty; d ++ )
					//if( dutyList[acTypeInd][k].demandInd[d] == fdInd )//found
					if( (demInd=dutyList[acTypeInd][k].demandInd[d]) >= 0 && demandList[demInd].fit == demandList[fitDemInd[fitCount]].fit )
					//&& ( fitDemInd[fitCount] == ( optParam.withFlexOS ? demandList[demInd].origDemInd : demInd)))//found
					{
						deptTm = 60*dutyList[acTypeInd][k].startTm[d];
						arrTm = deptTm + 60*demandList[demInd].elapsedTm[acTypeInd] ;
						break;
					}
				if( d < maxTripsPerDuty )
						break ;
			}
			if( j<MAX_WINDOW_DURATION )//found demand, check ac. If no ac, then get pickup ind and type of this tour
			{
				switch( pickupType = tours[i]->crewArcType )
				{ 
					case 1:
						acInd=crewPairList[tours[i]->crewPairInd].crewPlaneList[tours[i]->crewArcInd].acInd;
						break;
					case 2:
						acInd = crewPairList[tours[i]->crewPairInd].crewPUSList[tours[i]->crewArcInd]->acInd;
						pickupInd = crewPairList[tours[i]->crewPairInd].crewPUSList[tours[i]->crewArcInd]->demandInd;
						break;
					case 3:
						acInd = crewPairList[tours[i]->crewPairInd].crewPUEList[tours[i]->crewArcInd]->demandInd;
						pickupInd = crewPairList[tours[i]->crewPairInd].crewPUEList[tours[i]->crewArcInd]->demandInd;
						break;
					default:
						_ASSERTE(0==1);
				}
		...
		}//end for (i=0; i<nTours; i++)

		if( deptTm > 0 )
		{
			if( acInd < 0 )
			{
				_ASSERTE( pickupInd == 2 || pickupInd == 3);
				iteration = 0;
				while ( iteration < nTours)
				{
					for (i=0; i<nTours; i++)//go through each new tour, find previous tour ( before pickupInd)
					{
						acTypeInd = crewPairList[tours[i]->crewPairInd].acTypeIndex;
				
						j = optParam.planningWindowDuration - 1;
						while(tours[i]->duties[j] < 0)
							j--;

						if(( pickupType == 2 && pickupInd == dutyList[acTypeInd][tours[i]->duties[j]].repoDemandInd )
						|| ( pickupType == 3 && pickupInd == dutyList[acTypeInd][tours[i]->duties[j]].lastDemInd ))
						{
							switch( pickupType = tours[i]->crewArcType ) //update pickupType
							{ 
								case 1:
									acInd=crewPairList[tours[i]->crewPairInd].crewPlaneList[tours[i]->crewArcInd].acInd;
									break;
					...
					for (i=0; i<nExgTours; i++)
					{
						if( exgTours[i]->dropoffType == pickupType && exgTours[i]->dropoffInd == pickupInd )
						{
							acInd = exgTours[i]->acInd ;
							break;
						}
						if( exgTours[i]->dropoffType2 == pickupType && exgTours[i]->dropoffInd2 == pickupInd )
						{
							acInd = exgTours[i]->acInd2 ;
							break;
						}
					}
					if( acInd >= 0 )
			...
			docCost = ( (optParam.withMac && acList[acInd].isMac) ? acList[acInd].macDOC : acTypeList[acList[acInd].acTypeIndex].operatingCost) ;

			sprintf(outFitDemSql, "update fit_demand set proposeddeparturetime='%s', proposedarrivaltime='%s', proposedfleettype=%d, proposedac='%s' \n\
	, demanddoccost=%10.2f where recordid=%d "
			, dt_DateTimeToDateTimeString(dt_time_tToDateTime(deptTm), tmBuf1,"%Y/%m/%d %H:%M")
			, dt_DateTimeToDateTimeString(dt_time_tToDateTime(arrTm), tmBuf2,"%Y/%m/%d %H:%M")
			, acList[acInd].aircraftTypeID
			, acList[acInd].registration
			, docCost*(arrTm-deptTm)/3600.0
			, demandList[fitDemInd[fitCount]].fit );
		...
		} else //charter
		{
			for(acTypeInd=0; acTypeInd < numAcTypes; acTypeInd++)
				if( acTypeList[acTypeInd].aircraftTypeID == demandList[fitDemInd[fitCount]].aircraftTypeID )
					break ;
			_ASSERTE( acTypeInd < numAcTypes ) ;

			docCost = acTypeList[acTypeInd].operatingCost ;

			if( optParam.withFlexOS )
			{
				sprintf(outFitDemSql, "update fit_demand set proposeddeparturetime='%s', proposedarrivaltime='%s', proposedfleettype=%d \n\
				, demanddoccost=%10.2f where recordid=%d "
				, dt_DateTimeToDateTimeString(dt_time_tToDateTime(demandList[fitDemInd[fitCount]].origDefOut), tmBuf1,"%Y/%m/%d %H:%M")
				, dt_DateTimeToDateTimeString(dt_time_tToDateTime(demandList[fitDemInd[fitCount]].origDefIn), tmBuf2,"%Y/%m/%d %H:%M")
				, acTypeList[acTypeInd].aircraftTypeID
				, docCost*(demandList[fitDemInd[fitCount]].origDefIn-demandList[fitDemInd[fitCount]].origDefOut)/3600.0
				, demandList[fitDemInd[fitCount]].fit );
			...
			} else
			{
				sprintf(outFitDemSql, "update fit_demand set proposeddeparturetime='%s', proposedarrivaltime='%s', proposedfleettype=%d \n\
				, demanddoccost=%10.2f where recordid=%d "
				, dt_DateTimeToDateTimeString(dt_time_tToDateTime(demandList[fitDemInd[fitCount]].reqOut), tmBuf1,"%Y/%m/%d %H:%M")
				, dt_DateTimeToDateTimeString(dt_time_tToDateTime(demandList[fitDemInd[fitCount]].reqIn), tmBuf2,"%Y/%m/%d %H:%M")
				, acTypeList[acTypeInd].aircraftTypeID
				, docCost*(demandList[fitDemInd[fitCount]].reqIn-demandList[fitDemInd[fitCount]].reqOut)/3600.0
				, demandList[fitDemInd[fitCount]].fit );
			}
...
int showAllExgTours( const ExgTour *exgTourList, const int numExgTours )
{
	int i, j;

	for(i=0; i < numExgTours; i ++)
	{
		fprintf(logFile, "\n\n Existing Tour: %d, Cost: %f  --> \n \
AC1: %d, CPInd: %d, Crew: %d, Crew: %d, pickUpType: %d, pickUpInd: %d, dropOffType: %d, dropOffInd: %d \n Dem"
		, i, exgTourList[i].cost, acList[exgTourList[i].acInd].aircraftID, exgTourList[i].crewPairInd
		, crewPairList[exgTourList[i].crewPairInd].captainID, crewPairList[exgTourList[i].crewPairInd].flightOffID
		, exgTourList[i].pickupType, exgTourList[i].pickupInd, exgTourList[i].dropoffType, exgTourList[i].dropoffInd );

		for (j=0; j<MAX_LEGS; j++)
		{
			if( exgTourList[i].demandInd[j] >= 0 )
				fprintf(logFile, " %d --> ", demandList[exgTourList[i].demandInd[j]].demandID);
		}
		fprintf(logFile, "  \n" );

		fprintf(logFile, " AC2: %d, CPInd2: %d, Crew: %d, Crew: %d, pickUpType2: %d, pickUpInd2: %d, dropOffType2: %d, dropOffInd2: %d \n Dem"
		, ( exgTourList[i].acInd2 >= 0 ? acList[exgTourList[i].acInd2].aircraftID : 0 ), exgTourList[i].crewPairInd2
		, ( exgTourList[i].crewPairInd2 >= 0 ? crewPairList[exgTourList[i].crewPairInd2].captainID : 0 )
		, ( exgTourList[i].crewPairInd2 >= 0 ? crewPairList[exgTourList[i].crewPairInd2].flightOffID : 0 )
		, exgTourList[i].pickupType2, exgTourList[i].pickupInd2, exgTourList[i].dropoffType2, exgTourList[i].dropoffInd2 );

		for (j=0; j<MAX_LEGS; j++)
		{
			if( exgTourList[i].demandInd2[j] >= 0 )
				fprintf(logFile, " %d --> ", demandList[exgTourList[i].demandInd2[j]].demandID);
		}
		fprintf(logFile, "  \n" );
	}

	fflush(logFile); //06/22/2017 ANG
