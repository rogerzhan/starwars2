...
KW_Text kt[] = {
	...
	{ KW_Upgrade_Req_CX, "Upgrade Req. CX" }, //CX - 12/04/2009 ANG
...
AC_Convert acv[] = {
	...
	{ 7, 52,  KW_Upgrade_Req_Sov,   KW_Downgrade_Req_Sov,   4, 4 },  // Sovereign
	{ 8, 20,  KW_Upgrade_Req_CX,    KW_False,               5, 6 },  // CX - 12/04/2009 ANG
	{ 0,  0,  0,                    0,                      0, 0 }
};
...
static KeyWord downGradeReqKW_List[] = { KW_Downgrade_Req_90, KW_Downgrade_Req_200, KW_Downgrade_Req_CJ1, KW_Downgrade_Req_Bravo, 
	KW_Downgrade_Req_CJ3, KW_Downgrade_Req_Excel, KW_Downgrade_Req_Sov, KW_False }; //CX - 12/04/2009 ANG

static KeyWord upGradeReqKW_List[] = { KW_Upgrade_Req_200, KW_Upgrade_Req_Bravo, KW_Upgrade_Req_CJ1, KW_Upgrade_Req_Excel,
	KW_Upgrade_Req_Sov, KW_Upgrade_Req_CJ3, KW_Upgrade_Req_CX, KW_False }; //CX - 12/04/2009 ANG
...
static int readACTypeList(void)
{
	...
	if((paramVal = getCmdLineVar("maxUpgrades"))) {
		...
		if(wcUpgrades != 5) { //CX - 12/04/2009 ANG
			logMsg(logFile,"expected 5 \"maxUpgrades \" values, got %d instead.\n", wcUpgrades);
	...
	wcUpgrades = split(tbuf1," \t", wptrsUpgrades);
	//if(wcUpgrades != 4) {
	if(wcUpgrades != 5) { // CX - 12/04/2009
		logMsg(logFile,"expected 5 \"downgradeRecovery \" values, got %d instead.\n", wcUpgrades);
		//writeWarningData(myconn); exit(1);   //commented out for temp fix - CX - 12/21/2009 ANG
	}
	for(x = 0; x < wcUpgrades ; ++x) {
	...
	//if(wcUpgrades != 4) {
	if(wcUpgrades != 5) { // CX - 12/04/2009 ANG
		logMsg(logFile,"expected 5 \"upgradeRecovery \" values, got %d instead.\n", wcUpgrades);
		//writeWarningData(myconn); exit(1); //commented out for temp fix - CX - 12/21/2009 ANG
	}
	for(x = 0; x < wcUpgrades; ++x) {
...
static int readMaintList(MY_CONNECTION *myconn)
{
	...
	if(optParam.flexMxLoc == 1){
		char *mxflexlocSQL = "select * from smxlocation"; 
...
//	readNoFlexDemandList(&noFlexDemandNum, &onehrFlexDemandNum, &twohrFlexDemandNum);   
    readIrrFlexDemandList(&irrFlexDemandNum);   //Flex logic revised - 12/06/12 ANG
...
static int readExclusionType5(MY_CONNECTION *myconn)
{
	...
	BINTREENODE *tmp;
	char *sqlStmt = "select a.airportID, a.elevation, a.mainrunwaylength from tmp_apt t, airport a where a.airportID = t.airportid";
	//AircraftType *atPtr;
	...
	strcpy(getStateNamesSqlStmt, "select stateid, name from state where stateid in ("); //08/27/09 ANG
...
//THE FOLLOWING CAN BE simplified. RLZ: 10/15/2007
   ...
   //Correct any possible hasFlownFirst bug. No more than one crew pair can have "hasFlonwnFirst" on  the same aircraft.
	...
	if (tmp) {
		if (cpPtr->aircraftID[2] > 0) cpPtr->hasFlownFirst = 0;
		for(tmp = Successor(tmp); tmp; tmp = Successor(tmp)) {
			cpPtr1 = (CrewPair *) getTreeObject(tmp);
			if (cpPtr1->aircraftID[2] > 0) cpPtr1->hasFlownFirst = 0;
			if (*(cpPtr->aircraftID) == *(cpPtr1->aircraftID) && cpPtr->hasFlownFirst && cpPtr1->hasFlownFirst){
				//If both have flown the same ac
...
typedef enum { AcTypeBravo = 0, AcTypeCJ1, AcTypeCJ3, AcTypeExcel, AcTypeSovereign, AcTypeCX, AcTypeOther, AcTypeCount } AcTypeEnum; //CX - 12/04/2009 ANG
static int
analyseAircraftType(PRE_Crew *pre_crewPtr)
{
	...
	AcTypeEnum idx;
	AcTypeEnum thisType;
	int counts[] = { 0,0,0,0,0,0,0 }; //CX - 12/04/2009 ANG
	int returnValues[] = { 5, 6, 54, 11, 52, 20, 0 }; //CX - 12/04/2009 ANG
	char *acStr[] = { "Bravo", "CJ1", "CJ3", "Excel", "Sovereign", "CX", "Other" }; //CX - 12/04/2009 ANG
...
readPeakDays(MY_CONNECTION *myconn)
{
	...
	if(!myDoQuery(myconn, "select level_id, peak_day from cs_peak_days", &res, &cols)) { //New Vector - 05/26/10 ANG
		logMsg(logFile,"%s Line %d: db errno: %d: %s\n", __FILE__,__LINE__,myconn->my_errno, myconn->my_error_msg);
		writeWarningData(myconn); exit(1);
	}
...
readPeakDaysContractRates(MY_CONNECTION *myconn)
{
	...
	if(!myDoQuery(myconn, "select contractid, level_id, hourly_rate, flex_hours from cs_peak_days_contract_rates where hourly_rate is not null", &res, &cols)) {
		logMsg(logFile,"%s Line %d: db errno: %d: %s\n", __FILE__,__LINE__,myconn->my_errno, myconn->my_error_msg);
		writeWarningData(myconn); exit(1);
	}
...
//	mysql_free_result(res);
	...
	if(verbose) {
		...
		fprintf(logFile,"+--------+------+------+------+------+------+------+\n");
		fprintf(logFile,"| Demand |  CJ1 | Brav |  CJ3 |  Exc |  Sov |  CX  |\n"); //CX - 12/04/2009 ANG
		fprintf(logFile,"+--------+------+------+------+------+------+------+\n");
	}
	...
	// end of finding the right revenue for each demand RLZ 10/27/2010
		...
		if(verbose)
			fprintf(logFile,"| %6d | %4d | %4d | %4d | %4d | %4d | %4d |\n", diPtr->demandid,
				fcoBuf[0].blockTime, fcoBuf[1].blockTime, fcoBuf[2].blockTime, fcoBuf[3].blockTime, fcoBuf[4].blockTime, fcoBuf[5].blockTime); // CX - 12/04/2009 ANG
...
//			  for each plane above that: incremental = R * ratio_of_guaranteed_approved_plane * (time_in_flown - time_in_guaranteed)
	...
	fprintf(logFile,"| g |   CJ1 |  Brav |   CJ3 |   Exc |    Sov |     CX | Aircraft     | OwnerId | Id      | Rev/H   | Demand Out Time  | Out Time         |      Id |      Id | Name     | Pos | TYP | Name       | Pos | TYP | Name       | Keywords\n"); //CX - 12/04/2009 ANG
	fprintf(logFile,"+---+-------+-------+-------+-------+--------+--------+--------------+---------+---------+---------+------------------+------------------+---------+---------+----------+-----+-----+------------+-----+-----+------------+\n");
	...
	for(rows = 0; rows < rowCount; rows++) {
		...
		*/
		...
		if(tmp) {
			...
			if ( atoi(row[ud_seqpos]) < diPtr->seqpos && kwrt == kw_demand){
				diPtr->actypeid = atoi(row[ud_actypeid]);
				diPtr->seqpos = atoi(row[ud_seqpos]); 
...
getUpgradeDowngradeDataFromOracle(void)
{
	...
	while(fgets(buf, sizeof(buf) -1, inp)) {
		...
		if(tmp) {
			...
			if ( atoi(wptrs[ud_seqpos]) < diPtr->seqpos && kwrt == kw_demand){
				diPtr->actypeid = atoi(wptrs[ud_actypeid]);
				diPtr->seqpos = atoi(wptrs[ud_seqpos]); 
			}	// RLZ 10/28/2010 requested actype overwrite the upgrade in schedule. I believe we should further simply this by just looking at demand info
			continue;
		}
		else {
	 ...
	 while(Orlconnection_fetch(orl_socket)==0){
		...
		*/
		...
		if(tmp) {
			...
			if ( atoi(orl_socket->results[ud_seqpos]) < diPtr->seqpos && kwrt == kw_demand){
				diPtr->actypeid = atoi(orl_socket->results[ud_actypeid]);
				diPtr->seqpos = atoi(orl_socket->results[ud_seqpos]); 
			}	// RLZ 10/28/2010 requested actype overwrite the upgrade in schedule. I believe we should further simply this by just looking at demand info
			continue;
		}
		else {
...
typedef enum {
	...
	dmdnumCJ1,dmdnumBRV, dmdnumCJ3, dmdnumEXL, dmdnumSOV, dmdnumCX=5 // CX - 12/04/2009 ANG
	...
	for(rows = 0; rows < rowCount; rows++) {
		...
		if(DateTimeToTime_t(dt1) <= optParam.windowEnd && DateTimeToTime_t(dt2) >= optParam.windowStart) {//if there is overlap
			...
			switch (vacList[numVac].aircraftTypeID){ //may follow way to find sequencePosn in regular aircraft
				...
				case   11 : vacList[numVacInfo].sequencePosn = 6; break;
				case   52 : vacList[numVacInfo].sequencePosn = 7; break;
				case   20 : vacList[numVacInfo].sequencePosn = 8; break;
