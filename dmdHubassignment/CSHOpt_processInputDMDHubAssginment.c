...
static int DmdHubAssignment(void);// RLZ Hub assignment
...
/********************************************************************************
*	Function   DmdHubAssignment             Date last modified: 03/08/2012, RLZ	*
*	Purpose:   Assign neareast hub to dep airport and arr airport               *
********************************************************************************/
static int DmdHubAssignment(void)
{
	int i, j;
	int hubApt[] ={1978,4362,854,3453,1814,3281,1030,1454,1036,1110,3772,0};
    int stops, fltTm, blkTm, elapsedTm;
	int dptCompT, arrCompT;
	char tbuf1[32];
	char tbuf2[32];

	//Add code here for hub matching.
	...
	for(i=0; i<numDemand; i++)
	{
		...
		if (hubApt[j] == demandList[i].outAirportID){
			blkTm = 0;
		}
		else {
			getFlightTime(hubApt[j],demandList[i].outAirportID, demandList[i].aircraftTypeID, month, demandList[i].numPax,&fltTm, &blkTm, &elapsedTm, &stops);
		}
		dptCompT = blkTm;
        demandList[i].outHubID = hubApt[j];
		demandList[i].outHubTime = blkTm;

		if (demandList[i].inAirportID == hubApt[j]){
			blkTm = 0;
		}
		else {
			getFlightTime(demandList[i].inAirportID, hubApt[j], demandList[i].aircraftTypeID, month, demandList[i].numPax,&fltTm, &blkTm, &elapsedTm, &stops);
		}
		arrCompT = blkTm;
		demandList[i].inHubID = hubApt[j];
		demandList[i].inHubTime = blkTm;
		...
		logMsg(logFile,"demandList (with hubs):\n");
		fprintf(logFile,"\
+--------+--------+--------+-----+--------+--------+--------+--------+--------+--------+------------------+------------------+----------+----------+----------+-------+-------+-------+-------+-------+-----+-----|\n");
		fprintf(logFile,"\
| demand |  owner | contra | num | outAir | outApt | outFbo | inAir  | inApt  |  inFbo |                  |                  | contract | aircraft | inHubID  |   in  | outHub|  out  | leg       |\n");
		fprintf(logFile,"\
|     ID |     ID |     ID | Pax | portID | ICAO   |     ID | portID | ICAO   |     ID |           reqOut |            reqIn | Flag     | TypeID   |          | HubBT | ID    | HubBT | duration  |\n");
		fprintf(logFile,"\
+--------+--------+--------+-----+--------+--------+--------+--------+--------+--------+------------------+------------------+----------+----------+----------+-------+-------+-------+-------+-------+-----+-----|\n");
		for(i = 0; i < numDemand; i++) {
			fprintf(logFile,"| %6d | %6d | %6d | %3d | %6d | %-6s | %6d | %6d | %-6s | %6d | %s | %s | %8d | %8d | %8d | %5d | %5d | %5d | %5d |\n",
				demandList[i].demandID,
				demandList[i].ownerID,
				demandList[i].contractID,
				demandList[i].numPax,
				...
				demandList[i].contractFlag,
				demandList[i].aircraftTypeID,
				demandList[i].outHubID,
				demandList[i].outHubTime,
				demandList[i].inHubID,
				demandList[i].inHubTime,
				(demandList[i].reqIn - demandList[i].reqOut)/60); 
		}
		fprintf(logFile,"\
+--------+--------+--------+-----+--------+--------+--------+--------+--------+--------+------------------+------------------+----------+----------+----------+-------+-------+-------+-------+-------+-----+-----|\n");
		fflush(logFile);
