...
static int createOneTripNodes(int day)
{
	...
	for(i = firstdemandInd; i<=lastTripOfDay[day]; i++)
	{
		...
		//if(demandList[i].acInd >= -1)//Check this condition - FlexOS - 01/27/11 ANG
		...
		for(k = dutyTally[j][day][m-1].startInd; k<= dutyTally[j][day][m-1].endInd; k++)
		{
			...
			for(i = (day == 0? 0 : lastTripOfDay[day-1] + 1); i<=lastTripOfDay[day]; i++)
			{
				...
				if (!lastTrip->isAppoint){
					...
					//if(demandList[i].late[j] < (int)lastTrip->early[j]/60) //FlexOS - 01/27/11 ANG
						continue;				
				}
...
//					if(lastTrip->inFboID > 0 && lastTrip->outFboID > 0 && lastTrip->inFboID != newTrip->outFboID)
				...
				*newDuty = *oldDuty;
				...
				if (!newTrip->isAppoint)
					...
					//newDuty->endTm = (int)startTm[m]/60; //FlexOS - 01/27/11 ANG
				...
				for(i = lastTripOfDay[day]+ 1; i<=lastTripOfDay[day+1]; i++)  
				{
					...
					if(freshCrew == 1){
						...
						if(!demandList[i].isAppoint){
							...
							//if((repoEndTm + optParam.postFlightTm + optParam.minRestTm + optParam.preFlightTm) > demandList[i].late[j]) //found during FlexOS - 01/27/11 ANG
								continue;
						}
							...
							//if((repoEndTm + optParam.postFlightTm + optParam.minRestTm + optParam.preFlightTm) > ((int)(demandList[i].late[j] / 60) + demandList[i].elapsedTm[j]))//FlexOS - 01/27/11 ANG
								continue;
						}
					}
					//allocate memory for each duty and increment dutyTally
