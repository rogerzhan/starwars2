...
//static int readRecoveryDemandListHybrid(int* recoveryDemandNum, int* noFlexDemandNum); //read recovery demand list;
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	optParam.maxSecondDutyTm = atof(paramVal);
	...
	optParam.minSecondRestTm = atof(paramVal);
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	optParam.useOTFcrew = atof(paramVal);
...
static int readNoFlexDemandList(int* noFlexDemandNum, int* onehrFlexDemandNum, int* twohrFlexDemandNum)
{
	...
	/* 
	*noFlexDemandNum = split(tbuf," \t", wptrs);
	noFlexDemandList = (int *) calloc(*noFlexDemandNum, sizeof(int));
	if(! noFlexDemandList) {
	...
	for(x = 0; x < *noFlexDemandNum; x++) {
		noFlexDemandList[x] = atoi(wptrs[x]);
	}*/
...
/********************************************************************************
*	Function   readRecoveryDemandListHybrid              Date last modified: RLZ
*	Purpose:  Read in RecoveryDemand List from cmd.
********************************************************************************/
/*
static int readRecoveryDemandListHybrid(int* recoveryDemandNum, int* noFlexDemandNum)
{
	int x;	
	char tbuf[1024],*wptrs[128];
	char *paramVal;
	...
	if(verbose) logMsg(logFile,"recoveryDemandList or No Flex = %s\n", paramVal);
	strcpy(tbuf, paramVal);
	*recoveryDemandNum = split(tbuf," \t", wptrs);
	...
	{		
		*noFlexDemandNum = *recoveryDemandNum - 1;
		*recoveryDemandNum = 0;
		noFlexDemandList = (int *) calloc(*noFlexDemandNum, sizeof(int));
		if(! noFlexDemandList) {
			logMsg(logFile,"Out of Memory in readRecoveryDemandListHybrid().\n");
			writeWarningData(myconn); 
			exit(1);
		}

		for(x = 0; x < *noFlexDemandNum; x++) {
			noFlexDemandList[x] = atoi(wptrs[x+1]);
		}	
	}
...
typedef enum {
	...
	dmdOutcountryid, dmdIncountryid, dmdOutRunway, dmdInRunway, dmdRequires135, dmdIsV10Contract, 
...
//	readRecoveryDemandListHybrid(&recoveryDemandNum, &noFlexDemandNum);
    ...
    readNoFlexDemandList(&noFlexDemandNum, &onehrFlexDemandNum, &twohrFlexDemandNum);   //RLZ for no flex 02/26/2010
...
static int readExclusionType5(MY_CONNECTION *myconn)
{
	...
	if(numStateExcl > 0){
		...
		for(rows = 0; rows < rowCount; rows++) {
			...
			stateNameforExclList[numStateExcl] = strdup(row[1]);
			...
			if(optParam.withCTC){
			  ...
			  if(preCrewPtr->availDuringPlanningWindow == 0 && preCrewPtr->cscd_availDT > 0 ){ //&& preCrewPtr->canStartLater == 0){
				 if( !(preCrewPtr->lastActivityLeg) //lastActivityLeg is not populated
					 ...
					 && DateTimeToTime_t(dt_addToDateTime(Hours, 24*preCrewPtr->stayLate, preCrewPtr->tourEndTm)) > optParam.windowStart + 5*60*60){  
...
readPeakDays(MY_CONNECTION *myconn)
{
	...
	for(rows = 0; rows < rowCount; rows++) {
		...
		//if(!(pdPtr->shoulder_flag = strdup(row[2]))) { 
		//	logMsg(logFile,"%s Line %d, Out of Memory.\n",__FILE__,__LINE__);
		//	writeWarningData(myconn); exit(1);
		//}
...
readSsCrewData(MY_CONNECTION *myconn)
{
	...
	for(rows = 0; rows < rowCount; rows++) {
		...
		if(optParam.useOTFcrew == 0){ //12/29/09 ANG
			if((strcasecmp(strdup(row[SS_zacccodeid]),"OTF") == 0) || (strcasecmp(strdup(row[SS_zacccodeid]),"NAOT") == 0)){
...
readSsCrewDataFromOracleDB(ORACLE_SOCKET *orl_socket)
{
	...
	while(Orlconnection_fetch(orl_socket)==0){
		if(optParam.useOTFcrew == 0){ //12/29/09 ANG
			if((strcasecmp(strdup(orl_socket->results[zacccodeid]),"OTF") == 0) || (strcasecmp(strdup(orl_socket->results[zacccodeid]),"NAOT") == 0)){
		...
		if((tmp = TreeSearch(peakDayRoot, &pdBuf, peakDayCompare))) {
			...
			if(pdPtr->level_id == 1){
				...
				if((tmp1 = TreeSearch(peakDayContractRateRoot, &pdcrBuf, peakDayContractRatesCompare))) {
					...
					dPtr->earlyAdj = dPtr->lateAdj = (pdcrPtr->flex_hours * 60.0); //RLZ 05/05/2011
				}
			}
		}
		else if(dPtr->contractFlag == 1){
...
readDualQualCrew(MY_CONNECTION *myconn)
{
	...
	if(verbose) {
		...
		fprintf(logFile,"+--------+-----------+-----------+-----------+-----------+\n");
		for(a = 0; a < numDQCrew; a++) {
			fprintf(logFile,"| %6d | %9d | %9d | %9d | %9d |\n", dqCrewList[a].crewID, dqCrewList[a].acTypeID1, dqCrewList[a].acTypeID2);
		}
		fprintf(logFile,"+--------+-----------+-----------+-----------+-----------+\n");
		fflush(logFile);
	}
...
updateMacInfoWithVac (void)
{
	...
	MacInfo *newMacInfoList, *tPtr, *oldMacInfoList;
...
addVacInfoToMaintList(void)
{
	...
	for(x = 0; x < numVac; x++){
		...
		newMaintList[nmr].startTm = min(optParam.windowEnd - 10, vacInfoList[vacList[x].vacIndex].availEndTm);
		newMaintList[nmr].endTm = optParam.windowEnd + 60;
