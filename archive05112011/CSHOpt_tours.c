	...
	for(cp = 0; cp<numOptCrewPairs; cp++){
		...
		while(crewPairList[cp].schedLegIndList[y]> -1 && crewPairList[cp].schedLegACIndList[y]>-1){
			...
			if(i  > -1){
				...
				fprintf(logFile,"\nDemandID %d at time %d, late %d, early %d", demandList[i].demandID, (int) demandList[i].reqOut,demandList[i].lateAdj, demandList[i].earlyAdj);
				if(legList[lg].outAirportID == demandList[i].outAirportID && (((int)legList[lg].schedOut - (int)demandList[i].reqOut) > (demandList[i].lateAdj*60 + 60)) || 
					(((int)legList[lg].schedOut - (int)demandList[i].reqOut) < (-demandList[i].earlyAdj*60 - 60)))
				 {	fprintf(logFile,"\nDemandID %d on AircraftID %d has schedOut incompatible with reqOut.\n %d, dmd %d ", demandList[i].demandID, acList[acInd].aircraftID,(int)legList[lg].schedOut,(int)demandList[i].reqOut );
